...
`M-016 Runtime Authentication Implementation Boundary Planning` 已完成。当前 gate 是 `M-017 Authentication Generated Contract Shape Gate`，`W-0089 Add runtime authentication contract shape generator support and output` 是下一个 ready work item。`W-0088` 已决定 generated Go authentication contract shapes 应在 application authentication service interfaces 和 runtime authentication behavior 之前引入。`W-0089` 可以在 ADR-0038 path 下添加 generator/check support 和 metadata-only generated authentication shape files；它不授权 runtime authentication behavior、Protobuf messages、WebSocket behavior、token generation、token validation、verifier comparison、login、logout execution、refresh、cleanup jobs、authentication dependencies、repository changes 或 migration schema changes。
...
Authentication generated contract shape timing 记录在 `../docs/authentication-generated-contract-shape-timing.md` 和 `../decisions/ADR-0038-authentication-generated-contract-shape-timing.md`。Planned source 是 `contracts/runtime/authentication/`，planned output root 是 `runtime/internal/generated/contracts/runtime/authentication/`。Generated authentication shapes 是 metadata-only 且 immutable；service interfaces 和 runtime behavior 仍然单独 gated。

`../decisions/ADR-0037-close-runtime-auth-boundary-and-open-generated-shape-gate.md` 关闭 M-016 并打开 M-017。`ADR-0038` 完成 timing decision，并把 generator/check support 加 metadata-only generated authentication shape output 设为下一个 gated slice。
