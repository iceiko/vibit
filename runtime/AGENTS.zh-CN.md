...
`M-018 Application Authentication Service Interface Boundary` 已完成。`W-0090` 已定义未来 application-owned authentication service interface boundary，且没有添加 service code 或 runtime authentication behavior。当前 gate 是 `M-019 Token And Credential Verifier Algorithm Redaction Boundary`，`W-0091 Define token and credential verifier algorithm redaction boundary` 是下一个 ready work item。它可以定义 verifier algorithm posture、entropy、encoding、digest classification、constant-time comparison 和 redaction test expectations，但不得实现 token generation、verifier comparison、login、token validation、logout execution、refresh、cleanup jobs、Protobuf messages、WebSocket behavior、authentication dependencies、repository changes、migration schema changes 或 production authentication behavior。
...
`../decisions/ADR-0037-close-runtime-auth-boundary-and-open-generated-shape-gate.md` 关闭 M-016 并打开 M-017。`ADR-0038` 完成 timing decision。`W-0089` 完成 generator/check support 加 metadata-only generated authentication shape output。`ADR-0039` 和 `W-0090` 完成 service-interface boundary step，并打开 verifier algorithm/redaction preparation step。
