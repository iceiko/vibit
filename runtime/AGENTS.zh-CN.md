...
Selected login/token boundary checks 记录在 `../docs/selected-login-token-boundary-checks.md` 和 `../decisions/ADR-0030-selected-login-token-boundary-checks.md`。已选第一版姿态是 `device_credential_login`、opaque high-entropy access tokens、login-command token issuance、explicit request proof payloads、第一版不支持 refresh token，并为未来 credential 与 token verifier records 使用 PostgreSQL schema gates。该姿态仍然只是 semantic-contract-only。在未来 bounded implementation milestone 授权之前，不要添加 token validators、token issuers、credential repositories、token repositories、generated authentication contract shapes、authentication Protobuf sources、authentication migrations、WebSocket proof carriers 或 authentication dependencies。`runtime.selected_login_token_boundary` 是该已选姿态的 repository check rule。
...
Authentication schema migration queue planning 记录在 `../docs/authentication-schema-migration-queue.md` 和 `../decisions/ADR-0034-authentication-schema-migration-queue.md`。它排序 credential migration source、token verifier migration source、authentication migration static checks、repository-interface boundary、PostgreSQL adapter boundary 和 milestone closeout。除非后续 bounded work 明确授权 repository interfaces、adapters、runtime credential lookup、token issuance、token validation、logout、refresh、cleanup、handlers、routes、generated authentication shapes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior，否则 runtime code 必须保持不变。

`M-013 Login Method And Token Format Ratification` 已由 `ADR-0031` 完成。当前 gate 是 `M-014 Credential And Token Verifier Schema Ratification`，`W-0077 Add credential PostgreSQL migration source` 是下一个 ready work item。`W-0077` 只能添加已 ratified 的 `authentication_device_credentials` SQL-first migration source 以及相关 manifest/check updates；它不授权 token verifier tables、repositories、PostgreSQL adapters、runtime authentication behavior、Protobuf messages 或 WebSocket behavior。
