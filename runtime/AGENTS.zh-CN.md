...
`M-016 Runtime Authentication Implementation Boundary Planning` 已完成。当前 gate 是 `M-017 Authentication Generated Contract Shape Gate`，`W-0088 Decide authentication generated contract shape timing` 是下一个 ready work item。这个 gate 可以决定 generated Go authentication contract shapes 是否应在 application service interfaces 和 runtime authentication behavior 之前引入；它不授权创建 generated authentication shapes、runtime authentication behavior、Protobuf messages、WebSocket behavior、token generation、token validation、verifier comparison、login、logout execution、refresh、cleanup jobs 或 authentication dependencies。
...
`../decisions/ADR-0037-close-runtime-auth-boundary-and-open-generated-shape-gate.md` 关闭 M-016 并打开 M-017。W-0088 只能决定 generated authentication contract shape timing；它不得生成文件或添加 runtime behavior。
