...
`M-013 Login Method And Token Format Ratification` 已由 `ADR-0031` 完成。当前 gate 是 `M-014 Credential And Token Verifier Schema Ratification`，`W-0078 Add token verifier PostgreSQL migration source` 是下一个 ready work item。`W-0078` 只能添加已 ratified 的 `authentication_access_tokens` SQL-first migration source 以及相关 manifest/check updates；它不授权 repositories、PostgreSQL adapters、runtime authentication behavior、Protobuf messages 或 WebSocket behavior。
