...
`M-014 Credential And Token Verifier Schema Ratification` 已由 `ADR-0035` 完成。当前 gate 是 `M-015 Authentication PostgreSQL Adapter Implementation`。`W-0084 Implement authentication PostgreSQL adapter` 已完成，`W-0085 Close authentication PostgreSQL adapter implementation milestone` 是下一个 ready work item。关闭这个 milestone 只能 review 并记录已实现的 persistence adapter；它不授权 runtime authentication behavior、Protobuf messages、WebSocket behavior、generated authentication shapes、token generation、token validation、login、logout execution、refresh、cleanup jobs 或 authentication dependencies。
