...
`M-017 Authentication Generated Contract Shape Gate` is completed. The active gate is `M-018 Application Authentication Service Interface Boundary`, with `W-0090 Define application authentication service interface boundary` as the next ready work item. `W-0089` added generator/check support and metadata-only generated authentication shape files under `internal/generated/contracts/runtime/authentication/`; it does not authorize runtime authentication behavior, Protobuf messages, WebSocket behavior, token generation, token validation, verifier comparison, login, logout execution, refresh, cleanup jobs, authentication dependencies, repository changes, or migration schema changes.
...
`../decisions/ADR-0037-close-runtime-auth-boundary-and-open-generated-shape-gate.md` closes M-016 and opens M-017. `ADR-0038` completes the timing decision. `W-0089` completes generator/check support plus metadata-only generated authentication shape output and opens the next boundary-definition step.
