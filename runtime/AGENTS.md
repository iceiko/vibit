...
`M-014 Credential And Token Verifier Schema Ratification` is completed by `ADR-0035`. The active gate is `M-015 Authentication PostgreSQL Adapter Implementation`. `W-0084 Implement authentication PostgreSQL adapter` is completed, and `W-0085 Close authentication PostgreSQL adapter implementation milestone` is the next ready work item. Closing this milestone may review and record the implemented persistence adapter only; it does not authorize runtime authentication behavior, Protobuf messages, WebSocket behavior, generated authentication shapes, token generation, token validation, login, logout execution, refresh, cleanup jobs, or authentication dependencies.
