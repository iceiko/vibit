...
Selected login/token boundary checks are documented in `../docs/selected-login-token-boundary-checks.md` and `../decisions/ADR-0030-selected-login-token-boundary-checks.md`. The selected first posture is `device_credential_login` with opaque high-entropy access tokens, login-command token issuance, explicit request proof payloads, no refresh token in the first implementation, and PostgreSQL schema gates for future credential and token verifier records. This posture remains semantic-contract-only. Do not add token validators, token issuers, credential repositories, token repositories, generated authentication contract shapes, authentication Protobuf sources, authentication migrations, WebSocket proof carriers, or authentication dependencies until a later bounded implementation milestone authorizes them. `runtime.selected_login_token_boundary` is the repository check rule for this selected posture.

Credential record schema boundary is documented in `../docs/credential-record-schema-boundary.md` and `../decisions/ADR-0032-credential-record-schema-boundary.md`. It ratifies future `authentication_device_credentials` semantics for `device_credential_login` without adding schema or runtime behavior. Runtime code must remain unchanged until later bounded work explicitly authorizes migrations, repositories, adapters, runtime lookup, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.

Token verifier record schema boundary is documented in `../docs/token-verifier-record-schema-boundary.md` and `../decisions/ADR-0033-token-verifier-record-schema-boundary.md`. It ratifies future `authentication_access_tokens` semantics for opaque access-token verifier storage without adding schema or runtime behavior. Runtime code must remain unchanged until later bounded work explicitly authorizes migrations, repositories, adapters, runtime token issuance, validation, logout, refresh, cleanup, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.

Authentication schema migration queue planning is documented in `../docs/authentication-schema-migration-queue.md` and `../decisions/ADR-0034-authentication-schema-migration-queue.md`. It orders credential migration source, token verifier migration source, authentication migration static checks, repository-interface boundary, PostgreSQL adapter boundary, and milestone closeout. Runtime code must remain unchanged until a later bounded work item explicitly authorizes repository interfaces, adapters, runtime credential lookup, token issuance, token validation, logout, refresh, cleanup, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.

`M-013 Login Method And Token Format Ratification` is completed by `ADR-0031`. The active gate is `M-014 Credential And Token Verifier Schema Ratification`, with `W-0077 Add credential PostgreSQL migration source` as the next ready work item. `W-0077` may add only the ratified `authentication_device_credentials` SQL-first migration source and related manifest/check updates; it does not authorize token verifier tables, repositories, PostgreSQL adapters, runtime authentication behavior, Protobuf messages, or WebSocket behavior.
