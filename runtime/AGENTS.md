...
`M-013 Login Method And Token Format Ratification` is completed by `ADR-0031`. The active gate is `M-014 Credential And Token Verifier Schema Ratification`, with `W-0075 Define token verifier record schema boundary` as the next ready work item.
