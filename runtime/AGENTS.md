...
`M-013 Login Method And Token Format Ratification` is completed by `ADR-0031`. The active gate is `M-014 Credential And Token Verifier Schema Ratification`, with `W-0078 Add token verifier PostgreSQL migration source` as the next ready work item. `W-0078` may add only the ratified `authentication_access_tokens` SQL-first migration source and related manifest/check updates; it does not authorize repositories, PostgreSQL adapters, runtime authentication behavior, Protobuf messages, or WebSocket behavior.
