...
`M-181/W-0253 Implement Pitaya-aligned service discovery source-first map` 已完成。它接受 `ADR-0161`，注册 `runtime.pitaya_aligned_service_discovery_source_first_map`，实现 `node tools/vibit inspect pitaya-discovery --json`，并打开 `M-182/W-0254 Define Pitaya-aligned distributed group and broadcast boundary gate` 作为 next-ready。W-0254 必须保持 gate-only 和 source-first；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、service registries、service selectors、node identity、distributed groups、broadcast fanout、room broadcast fanout 或 cluster-safe session routing。
