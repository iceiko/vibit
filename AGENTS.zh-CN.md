...
在 `ADR-0158` 之后，近期优先级是 `M-179/W-0251 Implement Pitaya-aligned server-to-server RPC source-first map`。该 work 必须保持 source-first only；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、distributed groups、broadcast fanout 或 cluster-safe session routing。
