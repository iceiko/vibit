...
使用 `docs/v0.1-alpha-goal.md` 作为短期 release target intake。当前 release state 是 source-first `v0.1.0-alpha.1`，first alpha user discovery loop、feedback intake surface、product maturity milestones、prototype-ready execution plan、local development path gate、local development path package、storage objects behavior gate、storage objects persistence schema gate、storage objects migration source 和 storage objects repository boundary 已完成。目标是 `v0.1 alpha`，product maturity milestones 位于 `docs/product-maturity-milestones.md`，prototype-ready execution plan 位于 `docs/prototype-ready-foundation-execution-plan.md`，local development path gate 位于 `docs/prototype-ready-local-development-path-gate.md`，local development path package 位于 `docs/prototype-ready-local-development-path-package.md`，storage objects behavior gate 位于 `docs/storage-objects-behavior-gate.md`，storage objects persistence schema gate 位于 `docs/storage-objects-persistence-schema-gate.md`，storage objects migration source 是 `runtime/migrations/postgres/000006_create_storage_objects.sql`，storage objects repository boundary 位于 `docs/storage-objects-repository-boundary.md`，next work item 是 `W-0205 storage_objects_repository_interface_implementation`。Alpha acceptance checklist 位于 `docs/alpha-acceptance-checklist.md`，packaged local alpha developer flow 位于 `docs/alpha-developer-flow.md`，release publishing decision gate 位于 `docs/release-publishing-decision-gate.md`，release execution preparation gate 位于 `docs/release-execution-preparation-gate.md`，release execution authorization gate 位于 `docs/release-execution-authorization-gate.md`，release execution maintainer decision 位于 `docs/release-execution-maintainer-decision.md`，release identifier plan 位于 `docs/release-identifier-artifact-plan.md`，final authorization 位于 `docs/release-execution-final-authorization.md`，first alpha user discovery loop 位于 `docs/first-alpha-user-discovery-loop.md`，first alpha feedback intake 位于 `docs/first-alpha-feedback-intake-surfaces.md`。不要把 source-first alpha authorization、user discovery loop、feedback intake、prototype-ready execution plan、local development path gate、local development path package、storage objects behavior gate、storage objects persistence schema gate、storage objects migration source 或 storage objects repository boundary 理解为可以添加 release binaries、packages、containers、checksums、signing/provenance artifacts、hosted deployments、direct Nakama/Pitaya API compatibility、runtime behavior、protocol routes、generated output、dependencies、storage adapters、GitHub release record 之外的 public announcements、paid promotion，或跳过 `.arch/work-items.yaml`。
