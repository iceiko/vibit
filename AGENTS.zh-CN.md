...
在 `ADR-0159` 之后，近期优先级是 `M-180/W-0252 Define Pitaya-aligned service discovery boundary gate`。该 work 必须保持 gate-only；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、distributed groups、broadcast fanout 或 cluster-safe session routing。
