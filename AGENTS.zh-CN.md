...
`M-016 Runtime Authentication Implementation Boundary Planning` 已完成。当前 authentication-adjacent milestone 是 `M-017 Authentication Generated Contract Shape Gate`，`W-0088 Decide authentication generated contract shape timing` 是下一个 ready work item。这个 gate 可以决定 generated Go authentication contract shapes 是否应在 application service interfaces 和 runtime authentication behavior 之前引入；它不授权生成 authentication shapes、runtime token validation、token issuance、verifier comparison、logout execution、refresh、cleanup jobs、handlers、routes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior。
...
`ADR-0037` 关闭 runtime authentication implementation boundary planning milestone，并打开 generated authentication contract shape timing gate。这个 next gate 只能决定 generated shape timing；generated files 和 runtime behavior 仍然需要后续明确 work items 授权。
