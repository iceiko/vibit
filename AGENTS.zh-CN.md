...
使用 `docs/v0.1-alpha-goal.md` 作为短期 release target intake。当前 release state 是 pre-alpha，目标是 `v0.1 alpha`，即时 next ready work item 是 `W-0188 alpha_acceptance_checklist`。不要把该目标文档理解为发布 release、添加 direct Nakama/Pitaya API compatibility 或跳过 `.arch/work-items.yaml` 的授权。
...
`M-116 Alpha Acceptance Checklist` 当前 active。`W-0188` 是当前 next ready work item。它应添加 concise alpha acceptance checklist 或等价 check，覆盖 clone、prerequisites、migrations、local configuration、local onboarding posture、login、connection binding、protected inventory、presence query、logout、status endpoints、checks、redaction 和 contribution entry points。在该 slice 中，不要发布 `v0.1 alpha`、添加 release packaging、runtime behavior、protocol routes、Protobuf sources 或 generated output、migrations、dependencies、broad operations/admin behavior、authentication/session behavior changes、direct Nakama/Pitaya API compatibility 或 broad product modules。
