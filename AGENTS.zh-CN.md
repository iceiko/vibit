...
`M-184/W-0256 Define Pitaya-aligned cluster-safe session routing boundary gate` 已完成。它接受 `ADR-0164`，注册 `runtime.pitaya_aligned_cluster_safe_session_routing_boundary_gate`，定义 gate-only cluster-safe session routing vocabulary boundary，并打开 `M-185/W-0257 Implement Pitaya-aligned cluster-safe session routing source-first map` 作为 next-ready。W-0257 必须保持 source-first；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、service registries、service selectors、node identity、distributed groups、group membership registries、stream subscriptions、broadcast fanout、room broadcast fanout、delivery guarantees、cluster-safe session routing behavior、session location registries、connection owner node registries、routing epoch behavior、session route targets、remote connection handoff 或 distributed session routing。
