...
Credential record schema boundary standard 是 `docs/credential-record-schema-boundary.md`，配套简体中文译本是 `docs/credential-record-schema-boundary.zh-CN.md`。`ADR-0032` 记录 boundary decision。它 ratify `device_credential_login` 未来的 `authentication_device_credentials` semantics，但不授权 migrations、repositories、PostgreSQL adapters、runtime lookup、handlers、routes、generated authentication shapes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior。

Token verifier record schema boundary standard 是 `docs/token-verifier-record-schema-boundary.md`，配套简体中文译本是 `docs/token-verifier-record-schema-boundary.zh-CN.md`。`ADR-0033` 记录 boundary decision。它 ratify opaque access-token verifier storage 未来的 `authentication_access_tokens` semantics，但不授权 migrations、repositories、PostgreSQL adapters、runtime token issuance、validation、logout、refresh、cleanup、handlers、routes、generated authentication shapes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior。

Authentication schema migration queue standard 是 `docs/authentication-schema-migration-queue.md`，配套简体中文译本是 `docs/authentication-schema-migration-queue.zh-CN.md`。`ADR-0034` 记录 queue decision。它排序 credential migration source、token verifier migration source、authentication migration static checks、repository-interface boundary、PostgreSQL adapter boundary 和 milestone closeout；它不授权 runtime credential lookup、token issuance、token validation、logout、refresh、cleanup、handlers、routes、generated authentication shapes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior。

`M-013 Login Method And Token Format Ratification` 已由 `ADR-0031` 完成。当前 authentication-adjacent milestone 是 `M-014 Credential And Token Verifier Schema Ratification`，`W-0077 Add credential PostgreSQL migration source` 是下一个 ready work item。这个 next gate 只能添加已 ratified 的 `authentication_device_credentials` SQL-first migration source 以及相关 manifest/check updates；它不授权 token verifier tables、repositories、PostgreSQL adapters、runtime token validation、handlers、routes、generated authentication shapes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior。
