...
`M-013 Login Method And Token Format Ratification` 已由 `ADR-0031` 完成。当前 authentication-adjacent milestone 是 `M-014 Credential And Token Verifier Schema Ratification`，`W-0078 Add token verifier PostgreSQL migration source` 是下一个 ready work item。这个 next gate 只能添加已 ratified 的 `authentication_access_tokens` SQL-first migration source 以及相关 manifest/check updates；它不授权 repositories、PostgreSQL adapters、runtime token validation、token issuance、logout、refresh、cleanup、handlers、routes、generated authentication shapes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior。
