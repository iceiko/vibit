...
在 `ADR-0156` 之后，近期优先级是 `M-177/W-0249 Implement Pitaya-aligned frontend/backend role source-first map`。该 work 必须保持 source-first；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、distributed groups、broadcast fanout 或 cluster-safe session routing。
