...
`M-128 Prototype Ready Local Development Path Package` active。`W-0200 Implement prototype-ready local development path package` 是 next-ready。它应在 W-0199 gate 内打包 setup、migration、configuration/secret redaction、example-flow 和 verification ergonomics，再进入更宽的 product changes。
