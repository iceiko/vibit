...
在 `ADR-0153` 之后，近期优先级是 `M-174/W-0246 Define Pitaya-aligned distributed runtime vocabulary reactivation gate`。该 work 必须保持为 gate-only architecture vocabulary；除非后续 bounded work item 授权，不要实现 cluster/RPC/service-discovery behavior。
