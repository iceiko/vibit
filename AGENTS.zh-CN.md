...
使用 `.arch/reference.yaml` 作为 reference alignment 的机器可读 intake 入口。Nakama 是 broad game backend product capability surface 的主要产品能力参考。Pitaya 暂时降级为未来 distributed runtime concerns 的 architecture reference；在后续 ADR 重新激活前，不得驱动近期 cluster/RPC/frontend-backend 工作。改造 reference patterns 时必须保留 vibit 的 Agent-Native constraints，并记录为什么采纳、改造或拒绝某个 reference pattern。
...
使用 `docs/v0.1-alpha-goal.md` 作为短期 release target intake。当前 release state 是 source-first `v0.1.0-alpha.1`，first alpha user discovery loop、feedback intake surface、product maturity milestones、prototype-ready execution plan、local development path gate、local development path package、storage objects behavior gate、storage objects persistence schema gate、storage objects migration source、storage objects repository boundary、storage objects repository interface implementation、storage objects PostgreSQL adapter gate、storage objects PostgreSQL adapter implementation、storage objects runtime behavior gate、storage objects runtime behavior implementation、storage objects protocol route gate、storage objects protocol route implementation、storage objects protocol route local proof、storage objects local proof 后的 next-direction selection、first server push/realtime messaging gate、first server push and realtime messaging runtime slice、realtime runtime slice 后的 next-direction selection、realtime protocol and WebSocket outbound delivery gate、realtime protocol and WebSocket outbound delivery implementation，以及 realtime outbound delivery slice 后的 next-direction confirmation 已完成。目标是 `v0.1 alpha`，product maturity milestones 位于 `docs/product-maturity-milestones.md`，prototype-ready execution plan 位于 `docs/prototype-ready-foundation-execution-plan.md`，local development path gate 位于 `docs/prototype-ready-local-development-path-gate.md`，local development path package 位于 `docs/prototype-ready-local-development-path-package.md`，storage objects behavior gate 位于 `docs/storage-objects-behavior-gate.md`，storage objects persistence schema gate 位于 `docs/storage-objects-persistence-schema-gate.md`，storage objects migration source 是 `runtime/migrations/postgres/000006_create_storage_objects.sql`，storage objects repository boundary 位于 `docs/storage-objects-repository-boundary.md`，storage objects repository interface 是 `runtime/internal/modules/storage/repository.go`，storage objects PostgreSQL adapter gate 位于 `docs/storage-objects-postgresql-adapter-gate.md`，storage objects PostgreSQL adapter implementation 是 `runtime/internal/platform/persistence/postgres/storage_object_repository.go`，检查规则是 `runtime.storage_objects_postgresql_adapter_implementation`，storage objects runtime behavior gate 位于 `docs/storage-objects-runtime-behavior-gate.md`，检查规则是 `runtime.storage_objects_runtime_behavior_gate`，storage objects runtime behavior implementation 位于 `runtime/internal/app/storage/service.go`，检查规则是 `runtime.storage_objects_runtime_behavior_implementation`，storage objects protocol route gate 位于 `docs/storage-objects-protocol-route-gate.md`，检查规则是 `runtime.storage_objects_protocol_route_gate`，storage objects protocol route implementation 已记录在 `proto/vibit/storage/v1/storage.proto`、`runtime/internal/app/bootstrap/storage.go`、`runtime/internal/platform/protocol/protobuf/storage_bridge.go` 和 `decisions/ADR-0119-storage-objects-protocol-route-implementation.md`，检查规则是 `runtime.storage_objects_protocol_route_implementation`，storage objects protocol route local proof 已记录在 `runtime/internal/platform/protocol/protobuf/authenticated_gameplay_e2e_test.go`、`examples/local-alpha-request-loop.sh` 和 `decisions/ADR-0120-storage-objects-protocol-route-local-proof.md`，检查规则是 `runtime.storage_objects_protocol_route_local_proof`，storage objects local proof 后的 next-direction selection 已记录在 `decisions/ADR-0121-next-alpha-direction-first-server-push-realtime-messaging-gate.md`，检查规则是 `runtime.next_alpha_direction_after_storage_objects_local_proof`，first server push and realtime messaging gate 已记录在 `docs/first-server-push-realtime-messaging-gate.md` 和 `decisions/ADR-0122-first-server-push-realtime-messaging-gate.md`，检查规则是 `runtime.first_server_push_realtime_messaging_gate`，first server push and realtime messaging runtime slice 已记录在 `runtime/internal/app/realtime/service.go`、`runtime/internal/app/realtime/service_test.go` 和 `decisions/ADR-0123-first-server-push-realtime-messaging-runtime-slice.md`，检查规则是 `runtime.first_server_push_realtime_messaging_runtime_slice`，realtime runtime slice 后的 next-direction selection 已记录在 `decisions/ADR-0124-next-alpha-direction-realtime-protocol-websocket-outbound-delivery-gate.md`，检查规则是 `runtime.next_alpha_direction_after_realtime_runtime_slice`，realtime protocol and WebSocket outbound delivery gate 已记录在 `docs/realtime-protocol-websocket-outbound-delivery-gate.md` 和 `decisions/ADR-0125-realtime-protocol-websocket-outbound-delivery-gate.md`，检查规则是 `runtime.realtime_protocol_websocket_outbound_delivery_gate`，realtime protocol and WebSocket outbound delivery implementation 已记录在 `proto/vibit/realtime/v1/realtime.proto`、`runtime/internal/platform/protocol/protobuf/realtime_bridge.go`、`runtime/internal/platform/transport/ws/outbound.go` 和 `decisions/ADR-0126-realtime-protocol-websocket-outbound-delivery-implementation.md`，检查规则是 `runtime.realtime_protocol_websocket_outbound_delivery_implementation`，`M-147/W-0219 Confirm next alpha direction after realtime outbound delivery slice` 已由 `ADR-0127` 与检查规则 `runtime.next_alpha_direction_after_realtime_outbound_delivery_slice` 完成，next work item 是 `M-148/W-0220 Define agent-native feature request and test workflow`。Alpha acceptance checklist 位于 `docs/alpha-acceptance-checklist.md`，packaged local alpha developer flow 位于 `docs/alpha-developer-flow.md`，release publishing decision gate 位于 `docs/release-publishing-decision-gate.md`，release execution preparation gate 位于 `docs/release-execution-preparation-gate.md`，release execution authorization gate 位于 `docs/release-execution-authorization-gate.md`，release execution maintainer decision 位于 `docs/release-execution-maintainer-decision.md`，release identifier plan 位于 `docs/release-identifier-artifact-plan.md`，final authorization 位于 `docs/release-execution-final-authorization.md`，first alpha user discovery loop 位于 `docs/first-alpha-user-discovery-loop.md`，first alpha feedback intake 位于 `docs/first-alpha-feedback-intake-surfaces.md`。不要把 source-first alpha authorization、user discovery loop、feedback intake、prototype-ready execution plan、local development path gate、local development path package、storage objects behavior gate、storage objects persistence schema gate、storage objects migration source、storage objects repository boundary、storage objects repository interface implementation、storage objects PostgreSQL adapter gate、storage objects PostgreSQL adapter implementation、storage objects runtime behavior gate、storage objects runtime behavior implementation、storage objects protocol route gate、storage objects protocol route implementation、storage objects protocol route local proof、first server push and realtime messaging gate、first server push and realtime messaging runtime slice、realtime runtime slice 后的 next-direction selection、realtime protocol and WebSocket outbound delivery gate、realtime protocol and WebSocket outbound delivery implementation 或 realtime outbound delivery slice 后的 next-direction confirmation 理解为可以添加 release binaries、packages、containers、checksums、signing/provenance artifacts、hosted deployments、direct Nakama/Pitaya API compatibility、dependencies、additional storage adapters、additional SQL execution、GitHub release record 之外的 public announcements、paid promotion、production memory storage behavior、persistence、delivery guarantees、broad chat/social behavior、matchmaking、match runtime、distributed runtime，或跳过 `.arch/work-items.yaml`；下一 slice 只应定义 agent-native feature request and test workflow。
