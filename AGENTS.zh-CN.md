...
在 `ADR-0160` 之后，近期优先级是 `M-181/W-0253 Implement Pitaya-aligned service discovery source-first map`。该 work 必须保持 source-first；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、service registries、service selectors、node identity、distributed groups、broadcast fanout 或 cluster-safe session routing。
