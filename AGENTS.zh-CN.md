...
`M-017 Authentication Generated Contract Shape Gate` 已完成。当前 authentication-adjacent milestone 是 `M-018 Application Authentication Service Interface Boundary`，`W-0090 Define application authentication service interface boundary` 是下一个 ready work item。`W-0089` 已在 `runtime/internal/generated/contracts/runtime/authentication/` 下添加 generator/check support 和 metadata-only generated authentication shape files。这些 generated files 只是 source-traced metadata；它们不授权 runtime token validation、token issuance、verifier comparison、logout execution、refresh、cleanup jobs、handlers、routes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies、repository changes、migration schema changes 或 production authentication behavior。
...
`ADR-0037` 关闭 runtime authentication implementation boundary planning milestone，并打开 generated authentication contract shape timing gate。`ADR-0038` 完成 timing decision。`W-0089` 完成 generator/check support 加 metadata-only generated authentication shape output，并打开下一步 boundary-definition step。
