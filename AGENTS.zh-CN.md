...
`M-129 Storage Objects Behavior Gate` active。`W-0201 Define storage objects behavior gate` 是 next-ready。它应在 implementation 前定义 inventory proof slice 之外的第一版 general storage-object behavior。除非后续 bounded work 明确授权，不得实现 storage objects runtime behavior，不得添加 protocol routes、Protobuf sources 或 generated output、migrations、dependencies、repository/storage adapters、authentication/session changes、hosted deployments、release artifacts、public announcements、paid promotion、broad product modules 或 direct Nakama/Pitaya compatibility。
