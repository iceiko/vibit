...
`M-182/W-0254 Define Pitaya-aligned distributed group and broadcast boundary gate` 已完成。它接受 `ADR-0162`，注册 `runtime.pitaya_aligned_distributed_group_broadcast_boundary_gate`，定义 gate-only `distributed_group`、`room_broadcast`、`broadcast_target`、`group_membership` 和 `broadcast_fanout` vocabulary boundary，并打开 `M-183/W-0255 Implement Pitaya-aligned distributed group and broadcast source-first map` 作为 next-ready。W-0255 必须保持 source-first 和 inspection-only；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、service registries、service selectors、node identity、distributed groups、group membership registries、stream subscriptions、broadcast fanout、room broadcast fanout、delivery guarantees 或 cluster-safe session routing。
