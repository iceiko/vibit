...
`M-014 Credential And Token Verifier Schema Ratification` 已由 `ADR-0035` 完成。当前 authentication-adjacent milestone 是 `M-015 Authentication PostgreSQL Adapter Implementation`。`W-0084 Implement authentication PostgreSQL adapter` 已完成，`W-0085 Close authentication PostgreSQL adapter implementation milestone` 是下一个 ready work item。已实现的 adapter 只能是 persistence-adapter-only；它不授权 runtime token validation、token issuance、logout execution、refresh、cleanup jobs、handlers、routes、generated authentication shapes、Protobuf messages、WebSocket proof carriers、WebSocket handshake authentication、authentication dependencies 或 production authentication behavior。
