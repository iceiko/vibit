...
使用 `docs/v0.1-alpha-goal.md` 作为短期 release target intake。当前 release state 是 pre-alpha，目标是 `v0.1 alpha`，即时 next ready work item 是 `W-0192 release_execution_authorization_gate`。Alpha acceptance checklist 位于 `docs/alpha-acceptance-checklist.md`，packaged local alpha developer flow 位于 `docs/alpha-developer-flow.md`，release publishing decision gate 位于 `docs/release-publishing-decision-gate.md`，release execution preparation gate 位于 `docs/release-execution-preparation-gate.md`。不要把目标文档、checklist、packaged flow、release publishing decision gate 或 release execution preparation gate 理解为发布 release、添加 release packaging、创建 release artifacts、添加 direct Nakama/Pitaya API compatibility 或跳过 `.arch/work-items.yaml` 的授权。
