...
使用 `docs/v0.1-alpha-goal.md` 作为短期 release target intake。当前 release state 是 pre-alpha，目标是 `v0.1 alpha`，当前没有 immediate next ready work item，因为 `W-0193 release_execution_maintainer_decision` blocked，等待 maintainer 明确 go/no-go authorization。Alpha acceptance checklist 位于 `docs/alpha-acceptance-checklist.md`，packaged local alpha developer flow 位于 `docs/alpha-developer-flow.md`，release publishing decision gate 位于 `docs/release-publishing-decision-gate.md`，release execution preparation gate 位于 `docs/release-execution-preparation-gate.md`，release execution authorization gate 位于 `docs/release-execution-authorization-gate.md`。不要把目标文档、checklist、packaged flow、release publishing decision gate、release execution preparation gate 或 release execution authorization gate 理解为发布 release、添加 release packaging、选择 release identifiers、创建 release artifacts、添加 direct Nakama/Pitaya API compatibility 或跳过 `.arch/work-items.yaml` 的授权。
...
`M-121 Release Execution Maintainer Decision` 当前 active 且 blocked。`W-0193` blocked，直到 maintainer 明确选择 go/no-go 并授权任何 release identifier、tag、artifact、hosted deployment、publication surface 或 release execution command。没有该 explicit authorization 时，不要发布 `v0.1 alpha`、选择或创建 release identifiers 或 tags、创建 release binaries、archives、containers、packages、checksums、provenance files、hosted deployments、GitHub release records、runtime behavior、protocol routes、Protobuf sources 或 generated output、migrations、dependencies、broad operations/admin behavior、authentication/session behavior changes、direct Nakama/Pitaya API compatibility 或 broad product modules。

后续 major work 必须映射到一个 roadmap family：identity/auth/session、connection lifecycle、storage、presence/status/notifications、chat/realtime messaging、friends/groups/parties、leaderboards/tournaments、economy/progression、matchmaking、match runtime、server runtime hooks/RPC、operations、SDK/developer experience 或 distributed runtime。近期优先级现在是通向 `v0.1 alpha` 的最短路径：等待 release execution maintainer decision，然后用后续明确 work items 处理任何获批的 release-publishing 或 release-artifact actions。在 alpha path 可用前，不要直接跳到 chat、social modules、matchmaking、match runtime、SDKs、distributed runtime 或 direct Nakama/Pitaya API compatibility。
