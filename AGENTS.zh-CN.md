...
在 `ADR-0154` 之后，近期优先级是 `M-175/W-0247 Implement Pitaya-aligned distributed runtime vocabulary source-first map`。该 work 必须保持为 source-first vocabulary map；除非后续 bounded work item 授权，不要实现 cluster/RPC/service-discovery behavior。
