...
在 `ADR-0157` 之后，近期优先级是 `M-178/W-0250 Define Pitaya-aligned server-to-server RPC boundary gate`。该 work 必须保持 gate-only；除非后续 bounded work item 授权，不要实现 frontend/backend roles、cluster/RPC/service-discovery behavior、distributed groups、broadcast fanout 或 cluster-safe session routing。
