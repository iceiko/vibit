...
使用 `docs/v0.1-alpha-goal.md` 作为短期 release target intake。当前 release state 是 source-first `v0.1.0-alpha.1` authorization，目标是 `v0.1 alpha`，next work item 是 `W-0196 first_alpha_user_discovery`。Alpha acceptance checklist 位于 `docs/alpha-acceptance-checklist.md`，packaged local alpha developer flow 位于 `docs/alpha-developer-flow.md`，release publishing decision gate 位于 `docs/release-publishing-decision-gate.md`，release execution preparation gate 位于 `docs/release-execution-preparation-gate.md`，release execution authorization gate 位于 `docs/release-execution-authorization-gate.md`，release execution maintainer decision 位于 `docs/release-execution-maintainer-decision.md`，release identifier plan 位于 `docs/release-identifier-artifact-plan.md`，final authorization 位于 `docs/release-execution-final-authorization.md`。不要把 source-first alpha authorization 理解为可以添加 release binaries、packages、containers、checksums、signing/provenance artifacts、hosted deployments、direct Nakama/Pitaya API compatibility、runtime behavior、protocol routes、generated output、migrations、dependencies，或跳过 `.arch/work-items.yaml`。
...
`M-124 First Alpha User Discovery` active。`W-0196 Define first alpha user discovery loop` 是 next-ready。它应定义 target developer segments、outreach surfaces、feedback capture、success signals 和 stop conditions，同时不扩大 release artifacts 或 runtime scope。
