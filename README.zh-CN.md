...
使用 `.arch/work-items.yaml` 作为 continuation source of truth。`W-0231` 已完成 Nakama-first friends relationship lifecycle gate，并打开 friends relationship persistence schema gate。`continue` 或 `继续推进` 的意思是：推进一个 `next_ready` work item，除非遇到 ask-first boundary、verification failure 或 required maintainer confirmation。
