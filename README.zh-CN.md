...
使用 `.arch/work-items.yaml` 作为 continuation source of truth。`W-0231` 到 `W-0242` 已完成 friends relationship lifecycle、persistence、repository、PostgreSQL adapter、runtime behavior、protocol route 和 local proof path。`W-0243` 到 `W-0252` 已完成 friends route proof 后的 capability selection、minimum operations inspection gate、source-first operations inspection、Pitaya-aligned distributed runtime vocabulary gate、`node tools/vibit inspect pitaya-vocabulary --json`、frontend/backend role boundary gate、`node tools/vibit inspect pitaya-roles --json`、Pitaya-aligned server-to-server RPC boundary gate、`node tools/vibit inspect pitaya-rpc --json`，以及 Pitaya-aligned service discovery boundary gate。`ADR-0160` 已注册 `runtime.pitaya_aligned_service_discovery_boundary_gate`，并打开 `W-0253 Implement Pitaya-aligned service discovery source-first map`。下一方向是 `implement_pitaya_aligned_service_discovery_source_first_map`。`continue` 或 `继续推进` 的意思是：推进一个 `next_ready` work item，除非遇到 ask-first boundary、verification failure 或 required maintainer confirmation。
