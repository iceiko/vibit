...
使用 `.arch/work-items.yaml` 作为 continuation source of truth。`W-0231` 到 `W-0242` 已完成 friends relationship lifecycle、persistence、repository、PostgreSQL adapter、runtime behavior、protocol route 和 local proof path。`W-0243` 到 `W-0278` 已完成 friends route proof 后的 capability selection、minimum operations inspection gate、source-first operations inspection、Pitaya-aligned distributed runtime vocabulary gate、`node tools/vibit inspect pitaya-vocabulary --json`、frontend/backend role boundary gate、`node tools/vibit inspect pitaya-roles --json`、Pitaya-aligned server-to-server RPC boundary gate、`node tools/vibit inspect pitaya-rpc --json`、Pitaya-aligned service discovery boundary gate、`node tools/vibit inspect pitaya-discovery --json`、Pitaya-aligned distributed group and broadcast boundary gate、`node tools/vibit inspect pitaya-groups --json`、Pitaya-aligned cluster-safe session routing boundary gate、`node tools/vibit inspect pitaya-sessions --json`、下一项 Pitaya-aligned direction selection、Pitaya-aligned route handler pipeline boundary gate、`node tools/vibit inspect pitaya-routes --json`、route handler pipeline map 后的下一项 Pitaya-aligned direction selection、Pitaya-aligned serializer and message forwarding boundary gate、`node tools/vibit inspect pitaya-serializer-forwarding --json`、serializer and message forwarding map 后的下一项 Pitaya-aligned direction selection、Pitaya-aligned acceptor and connection lifecycle boundary gate、`node tools/vibit inspect pitaya-acceptor-connection --json`、acceptor and connection lifecycle map 后的下一项 Pitaya-aligned direction selection、session binding/kick-disconnect/session data boundary gate、`node tools/vibit inspect pitaya-session-lifecycle --json`、session lifecycle map 后的下一项 Pitaya-aligned direction selection、Pitaya-aligned runtime observability boundary gate、`node tools/vibit inspect pitaya-observability --json`、runtime observability map 后的下一项 Pitaya-aligned direction selection、Pitaya-aligned metrics and tracing boundary gate、`node tools/vibit inspect pitaya-metrics-tracing --json`、metrics/tracing map 后的下一项 Pitaya-aligned direction selection、Pitaya-aligned dashboard and admin operations boundary gate、`node tools/vibit inspect pitaya-dashboard-admin --json`、Pitaya-aligned runtime component lifecycle boundary gate，以及 `node tools/vibit inspect pitaya-component-lifecycle --json`。`ADR-0192` 已注册 `runtime.pitaya_aligned_handler_module_registration_source_first_map`，实现 `node tools/vibit inspect pitaya-handler-modules --json`，并完成 `W-0284 Implement Pitaya-aligned handler module registration source-first map`；`ADR-0193` 注册 `runtime.next_pitaya_aligned_direction_after_handler_module_registration_map`，完成 `W-0285`，选择 `define_pitaya_aligned_component_discovery_module_loading_boundary_gate`，并打开 `W-0286 Define Pitaya-aligned component discovery and module loading boundary gate`；`ADR-0194` 注册 `runtime.pitaya_aligned_component_discovery_module_loading_boundary_gate`，完成 `W-0286`，并打开 `W-0287 Implement Pitaya-aligned component discovery and module loading source-first map`；`ADR-0195` 注册 `runtime.pitaya_aligned_component_discovery_module_loading_source_first_map`，实现 `node tools/vibit inspect pitaya-component-loading --json`，完成 `W-0287`，并打开 `W-0288 Select next Pitaya-aligned direction after component discovery and module loading map`；`ADR-0196` 注册 `runtime.next_pitaya_aligned_direction_after_component_discovery_module_loading_map`，完成 `W-0288`，选择 `define_pitaya_aligned_startup_shutdown_hook_boundary_gate`，并打开 `W-0289 Define Pitaya-aligned startup and shutdown hook boundary gate`；`ADR-0197` 注册 `runtime.pitaya_aligned_startup_shutdown_hook_boundary_gate`，完成 `W-0289`，并打开 `W-0290 Implement Pitaya-aligned startup and shutdown hook source-first map`。下一方向是 `implement_pitaya_aligned_startup_shutdown_hook_source_first_map`。`continue` 或 `继续推进` 的意思是：推进一个 `next_ready` work item，除非遇到 ask-first boundary、verification failure 或 required maintainer confirmation。
