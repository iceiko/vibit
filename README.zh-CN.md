...
使用 `.arch/work-items.yaml` 作为 continuation source of truth。`W-0231` 到 `W-0242` 已完成 friends relationship lifecycle、persistence、repository、PostgreSQL adapter、runtime behavior、protocol route 和 local proof path。`W-0243` 到 `W-0278` 已完成 friends route proof 后的 capability selection 与 Pitaya-aligned inspection/boundary maps。`W-0279` 到 `W-0291` 已完成 Pitaya-aligned runtime component、handler module、component loading、startup/shutdown 和 next-direction work。`W-0292` 到 `W-0302` 已完成 currency wallet lifecycle boundary、persistence schema gate、migration source、repository boundary、storage-neutral repository interface implementation、PostgreSQL adapter gate、PostgreSQL adapter implementation、runtime behavior gate、application-owned runtime behavior implementation、protocol route gate 和 protocol route implementation。`ADR-0209` 注册 `runtime.currency_wallet_protocol_route_gate` 并定义 `docs/currency-wallet-protocol-route-gate.md`；`ADR-0210` 注册 `runtime.currency_wallet_protocol_route_implementation`，添加 `proto/vibit/currency/v1/currency.proto`、generated Go output、route keys、bootstrap handlers、protocol bridge mapping 和 local authenticated route proof。下一方向是 `no_next_ready_work_item`。`continue` 或 `继续推进` 的意思是：推进一个 `next_ready` work item，除非遇到 ask-first boundary、verification failure、required maintainer confirmation 或 no next-ready work item。
...
`ADR-0210` 注册 `runtime.currency_wallet_protocol_route_implementation`，完成 `W-0302 Implement currency wallet protocol route`，添加 `proto/vibit/currency/v1/currency.proto`、generated Go output、route keys、bootstrap handlers、protocol bridge mapping 和 local authenticated route proof。`ADR-0209` 仍是 `decisions/ADR-0209-currency-wallet-protocol-route-gate.md` 中的 route gate decision。

下一方向是 `no_next_ready_work_item`。


当前 currency wallet repository boundary decision：`decisions/ADR-0203-currency-wallet-repository-boundary.md`；当前 repository interface decision：`decisions/ADR-0204-currency-wallet-repository-interface-implementation.md`；当前 PostgreSQL adapter gate decision：`decisions/ADR-0205-currency-wallet-postgresql-adapter-gate.md`；当前 PostgreSQL adapter implementation decision：`decisions/ADR-0206-currency-wallet-postgresql-adapter-implementation.md`；当前 runtime behavior gate decision：`decisions/ADR-0207-currency-wallet-runtime-behavior-gate.md`；当前 runtime behavior implementation decision：`decisions/ADR-0208-currency-wallet-runtime-behavior-implementation.md`；当前 protocol route gate decision：`decisions/ADR-0209-currency-wallet-protocol-route-gate.md`；当前 protocol route implementation decision：`decisions/ADR-0210-currency-wallet-protocol-route-implementation.md`；下一 work item：`No repository next work item is currently ready`。
