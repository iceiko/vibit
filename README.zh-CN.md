...
使用 `.arch/work-items.yaml` 作为 continuation source of truth。`W-0231` 到 `W-0242` 已完成 friends relationship lifecycle、persistence、repository、PostgreSQL adapter、runtime behavior、protocol route 和 local proof path。`W-0243` 到 `W-0249` 已完成 friends route proof 后的 capability selection、minimum operations inspection gate、source-first operations inspection、Pitaya-aligned distributed runtime vocabulary gate、`node tools/vibit inspect pitaya-vocabulary --json`、frontend/backend role boundary gate，以及 `node tools/vibit inspect pitaya-roles --json`。`ADR-0157` 已注册 `runtime.pitaya_aligned_frontend_backend_role_source_first_map`，并打开 `W-0250 Define Pitaya-aligned server-to-server RPC boundary gate`。下一方向是 `define_pitaya_aligned_server_to_server_rpc_boundary_gate`。`continue` 或 `继续推进` 的意思是：推进一个 `next_ready` work item，除非遇到 ask-first boundary、verification failure 或 required maintainer confirmation。
