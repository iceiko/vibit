...
使用 `.arch/work-items.yaml` 作为 continuation source of truth。`W-0231` 已完成 Nakama-first friends relationship lifecycle gate。`W-0232` 已完成 friends relationship persistence schema gate。`W-0233` 已添加 `runtime/migrations/postgres/000007_create_friend_relationships.sql` 中的 friends relationship migration source，并接受 `decisions/ADR-0141-friends-relationship-migration-source.md`。`W-0234` 已定义 `docs/friends-relationship-repository-boundary.md`，接受 `decisions/ADR-0142-friends-relationship-repository-boundary.md`。`W-0235` 已实现 storage-neutral friends relationship repository interface，接受 `decisions/ADR-0143-friends-relationship-repository-interface-implementation.md`，并注册 `runtime.friends_relationship_repository_interface_implementation`。`W-0236` 已定义 `docs/friends-relationship-postgresql-adapter-gate.md`，接受 `decisions/ADR-0144-friends-relationship-postgresql-adapter-gate.md`，并注册 `runtime.friends_relationship_postgresql_adapter_gate`。`W-0237` 已实现 friends relationship PostgreSQL adapter，接受 `decisions/ADR-0145-friends-relationship-postgresql-adapter-implementation.md`，并注册 `runtime.friends_relationship_postgresql_adapter_implementation`。`W-0238` 已定义 friends relationship runtime behavior gate，接受 `decisions/ADR-0146-friends-relationship-runtime-behavior-gate.md`，并注册 `runtime.friends_relationship_runtime_behavior_gate`。`W-0239` 已实现 friends relationship runtime behavior service，接受 `decisions/ADR-0147-friends-relationship-runtime-behavior-implementation.md`，并注册 `runtime.friends_relationship_runtime_behavior_implementation`。`W-0240` 已定义 friends relationship protocol route gate，接受 `decisions/ADR-0148-friends-relationship-protocol-route-gate.md`，并注册 `runtime.friends_relationship_protocol_route_gate`。`W-0241` 已实现 friends relationship protocol route family，接受 `decisions/ADR-0149-friends-relationship-protocol-route-implementation.md`，并注册 `runtime.friends_relationship_protocol_route_implementation`。`W-0242` 已证明 friends relationship protocol route family 的 local flow，接受 `decisions/ADR-0150-friends-relationship-protocol-route-local-proof.md`，并注册 `runtime.friends_relationship_protocol_route_local_proof`。下一方向是 `select_next_nakama_prototype_ready_capability_after_friends_relationship_route_proof`。`continue` 或 `继续推进` 的意思是：推进一个 `next_ready` work item，除非遇到 ask-first boundary、verification failure 或 required maintainer confirmation。
