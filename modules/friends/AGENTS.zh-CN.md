...
repository 下一项 work item 是 `W-0250 Define Pitaya-aligned server-to-server RPC boundary gate`。该 work 不属于本模块且必须保持 gate-only；在后续 bounded work item 明确授权前，不要在本模块添加 protocol shape changes、repository interface changes、PostgreSQL adapter changes、migrations、dependencies、event/audit tables、groups、parties、chat、matchmaking、match runtime、SDK publication、hosted deployments、distributed runtime behavior、frontend/backend server role implementation、server-to-server RPC behavior、operations/admin implementation 或 direct Nakama/Pitaya API compatibility。
