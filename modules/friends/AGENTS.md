...
The repository next work item is `W-0255 Implement Pitaya-aligned distributed group and broadcast source-first map`. That work is outside this module and must stay source-first and inspection-only; do not add protocol shape changes, repository interface changes, PostgreSQL adapter changes, migrations, dependencies, event/audit tables, groups, parties, chat, matchmaking, match runtime, SDK publication, hosted deployments, distributed runtime behavior, frontend/backend server role implementation, server-to-server RPC behavior, remote calls, service discovery implementation, service registries, service selectors, distributed groups, group membership registries, stream subscriptions, room broadcast fanout, broadcast delivery guarantees, operations/admin implementation, or direct Nakama/Pitaya API compatibility in this module until a later bounded work item authorizes that scope.
...
- Next Pitaya-aligned vocabulary follow-up: `W-0255 Implement Pitaya-aligned distributed group and broadcast source-first map`
