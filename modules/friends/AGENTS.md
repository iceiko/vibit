...
`ADR-0182` registered `runtime.pitaya_aligned_metrics_tracing_boundary_gate`, completed `W-0274 Define Pitaya-aligned metrics and tracing boundary gate`, selected `implement_pitaya_aligned_metrics_tracing_source_first_map`, and opened `W-0275 Implement Pitaya-aligned metrics and tracing source-first map` as the repository next work item. The `node tools/vibit inspect pitaya-observability --json` inspection remains available. This remains outside the friends module.
