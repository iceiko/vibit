...
`ADR-0189` registered `runtime.pitaya_aligned_runtime_component_lifecycle_source_first_map`, completed `W-0281 Implement Pitaya-aligned runtime component lifecycle source-first map`, implemented `node tools/vibit inspect pitaya-component-lifecycle --json`, and opened `W-0282 Select next Pitaya-aligned direction after runtime component lifecycle map` as the repository next work item. This remains outside the friends module and does not add dashboard, admin, runtime endpoint, protocol, generated output, persistence, dependency, distributed runtime, or direct compatibility scope.
