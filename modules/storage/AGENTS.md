...
Storage module storage-object work is complete through local proof, the first realtime runtime slice is implemented outside this module, the realtime protocol/WebSocket outbound delivery gate and implementation are completed outside this module, the agent-native feature request and test workflow is defined outside this module, and the Nakama-aligned workflow pilot selected presence/status work outside this module. The repository next work item is `W-0222 Harden presence status local proof through close and offline cases`, outside this module. Do not use the storage module as owner for realtime protocol, WebSocket outbound delivery, repository-wide feature workflow behavior, or presence/status proof hardening; preserve storage service, repository interface, PostgreSQL adapter, migration, public ACL, admin search, group/guild/party/room/match storage scope, batch write, JSON patch, merge, TTL, script hook, large object/blob storage, S3-compatible object storage, production memory storage behavior, and direct Nakama/Pitaya API compatibility deferrals unless a later bounded storage work item authorizes them.
