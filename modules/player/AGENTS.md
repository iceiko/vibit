...
`M-109 Local Onboarding Device Credential Issuance Gate` is completed. `W-0181` defined `docs/local-onboarding-device-credential-issuance-gate.md` and `ADR-0089`. Future local onboarding may call this module's `CreatePlayerAccount` repository method through an application unit-of-work, but the player module remains storage-neutral player account lifecycle ownership only. Player account tables must remain credential-free, token-free, provider-subject-free, session-free, and WebSocket-state-free. This module must not generate credentials, store credential digests, decide authentication outcomes, expose onboarding routes, add migrations, add dependencies, add production signup, add external identity providers, add password login, add account recovery, add multi-device linking, or add direct Nakama/Pitaya API compatibility. `runtime.local_onboarding_device_credential_issuance_gate` is the repository check rule.

`M-110 Local Onboarding Device Credential Issuance Implementation` is completed. `W-0182` implemented local onboarding in the application service under `runtime/internal/app/authentication`, using this module's existing `CreatePlayerAccount` repository method to create the player lifecycle row in the same unit of work as digest-only credential storage. This module did not gain ownership of credential generation, credential digest storage, authentication outcomes, onboarding routes, migrations, dependencies, production signup, external identity providers, password login, account recovery, multi-device linking, or direct Nakama/Pitaya API compatibility. Player lifecycle tables remain credential-free, token-free, provider-subject-free, session-free, and WebSocket-state-free. `runtime.local_onboarding_device_credential_issuance_implementation` is the repository check rule. The current next ready work item is `M-111/W-0183 select_next_alpha_direction_after_local_onboarding`.
