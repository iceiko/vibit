...
`M-109 Local Onboarding Device Credential Issuance Gate` 已完成。`W-0181` 定义了 `docs/local-onboarding-device-credential-issuance-gate.md` 和 `ADR-0089`。Future local onboarding 后续可以通过 application unit-of-work 调用本模块的 `CreatePlayerAccount` repository method，但 player module 仍然只拥有 storage-neutral player account lifecycle。Player account tables 必须保持 credential-free、token-free、provider-subject-free、session-free 和 WebSocket-state-free。本模块不得生成 credentials、存储 credential digests、决定 authentication outcomes、暴露 onboarding routes、添加 migrations、添加 dependencies、添加 production signup、添加 external identity providers、添加 password login、添加 account recovery、添加 multi-device linking 或添加 direct Nakama/Pitaya API compatibility。`runtime.local_onboarding_device_credential_issuance_gate` 是 repository check rule。

`M-110 Local Onboarding Device Credential Issuance Implementation` 已完成。`W-0182` 在 `runtime/internal/app/authentication` application service 中实现了 local onboarding，并使用本模块已有 `CreatePlayerAccount` repository method，在同一个 unit of work 中创建 player lifecycle row 和 digest-only credential storage。本模块没有获得 credential generation、credential digest storage、authentication outcomes、onboarding routes、migrations、dependencies、production signup、external identity providers、password login、account recovery、multi-device linking 或 direct Nakama/Pitaya API compatibility 的所有权。Player lifecycle tables 仍保持 credential-free、token-free、provider-subject-free、session-free 和 WebSocket-state-free。`runtime.local_onboarding_device_credential_issuance_implementation` 是 repository check rule。当前 next ready work item 是 `M-111/W-0183 select_next_alpha_direction_after_local_onboarding`。
