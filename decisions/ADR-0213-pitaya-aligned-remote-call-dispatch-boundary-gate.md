# ADR-0213: Pitaya-Aligned Remote Call Dispatch Boundary Gate
...
- `conversations/2026-06-13-define-pitaya-aligned-remote-call-dispatch-boundary-gate.md`
...
Define `docs/pitaya-aligned-remote-call-dispatch-boundary-gate.md` and register `runtime.pitaya_aligned_remote_call_dispatch_boundary_gate` as the gate-only repository check rule for `M-233/W-0305`.

This decision records `W-0305` as completed, with `ADR-0213` as the decision and `runtime.pitaya_aligned_remote_call_dispatch_boundary_gate` as the check rule.
...
Remote Call Dispatch is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow gate lets agents reason about future architecture without changing the running server.
