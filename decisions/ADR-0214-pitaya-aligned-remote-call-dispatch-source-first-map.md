# ADR-0214: Pitaya-Aligned Remote Call Dispatch Source-First Map
...
- `changes/2026-06-13-implement-pitaya-aligned-remote-call-dispatch-source-first-map/`

Related conversations:

- `conversations/2026-06-13-implement-pitaya-aligned-remote-call-dispatch-source-first-map.md`
...
Implement `node tools/vibit inspect pitaya-remote-call-dispatch --json` and register `runtime.pitaya_aligned_remote_call_dispatch_source_first_map` as the source-first map check rule for `M-234/W-0306`.

This decision records `W-0306` as completed, with `ADR-0214` as the decision and `runtime.pitaya_aligned_remote_call_dispatch_source_first_map` as the check rule.

This decision does not add remote call dispatch behavior, server-to-server RPC behavior, remote timeout or retry behavior, runtime behavior, protocol messages or routes, Protobuf source, generated output, repository interfaces, PostgreSQL adapters, migrations, dependencies, authentication/session behavior changes, hosted deployments, SDK publication, release artifacts, distributed runtime behavior, or direct Nakama/Pitaya API compatibility.

## Alternatives Considered

- Implement remote call dispatch behavior immediately.
...
Remote Call Dispatch is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow source-first inspection map lets agents reason about future architecture without changing the running server.
...
Agents can now inspect or reason about Pitaya-aligned remote call dispatch vocabulary without treating that vocabulary as live runtime behavior.

The repository remains single-process and source-first for this concern.

## Reversal Conditions

Revisit this decision if a later ADR selects a concrete remote call dispatch, distributed runtime, service discovery, RPC, frontend/backend, event bus, protocol, persistence, hosted, or direct compatibility implementation model.
