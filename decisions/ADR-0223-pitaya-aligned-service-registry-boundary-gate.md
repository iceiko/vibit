# ADR-0223: Pitaya-Aligned Service Registry Boundary Gate
...
- `conversations/2026-06-14-define-pitaya-aligned-service-registry-boundary-gate.md`
...
Define `docs/pitaya-aligned-service-registry-boundary-gate.md` and register `runtime.pitaya_aligned_service_registry_boundary_gate` as the gate-only repository check rule for `M-243/W-0315`.

This decision records `W-0315` as completed, with `ADR-0223` as the decision and `runtime.pitaya_aligned_service_registry_boundary_gate` as the check rule.
...
Service Registry is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or operations behavior. A narrow gate lets agents reason about future architecture without changing the running server.
