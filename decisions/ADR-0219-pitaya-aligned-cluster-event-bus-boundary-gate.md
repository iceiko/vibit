# ADR-0219: Pitaya-Aligned Cluster Event Bus Boundary Gate
...
- `changes/2026-06-13-define-pitaya-aligned-cluster-event-bus-boundary-gate/`

Related conversations:

- `conversations/2026-06-13-define-pitaya-aligned-cluster-event-bus-boundary-gate.md`
...
Define `docs/pitaya-aligned-cluster-event-bus-boundary-gate.md` and register `runtime.pitaya_aligned_cluster_event_bus_boundary_gate` as the gate-only repository check rule for `M-239/W-0311`.

This decision records `W-0311` as completed, with `ADR-0219` as the decision and `runtime.pitaya_aligned_cluster_event_bus_boundary_gate` as the check rule.
...
Cluster Event Bus is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow gate lets agents reason about future architecture without changing the running server.
