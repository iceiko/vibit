# ADR-0218: Pitaya-Aligned Backend Service Route Ownership Source-First Map
...
- `changes/2026-06-13-implement-pitaya-aligned-backend-service-route-ownership-source-first-map/`

Related conversations:

- `conversations/2026-06-13-implement-pitaya-aligned-backend-service-route-ownership-source-first-map.md`

Related artifacts:

- `docs/pitaya-aligned-backend-service-route-ownership-boundary-gate.md`
- `docs/pitaya-aligned-backend-service-route-ownership-boundary-gate.zh-CN.md`
...
Implement `node tools/vibit inspect pitaya-backend-service-routes --json` and register `runtime.pitaya_aligned_backend_service_route_ownership_source_first_map` as the source-first map check rule for `M-238/W-0310`.

This decision records `W-0310` as completed, with `ADR-0218` as the decision and `runtime.pitaya_aligned_backend_service_route_ownership_source_first_map` as the check rule.

This decision does not add backend service route ownership behavior, backend service targeting behavior, distributed route conflict behavior, runtime behavior, protocol messages or routes, Protobuf source, generated output, repository interfaces, PostgreSQL adapters, migrations, dependencies, authentication/session behavior changes, hosted deployments, SDK publication, release artifacts, distributed runtime behavior, or direct Nakama/Pitaya API compatibility.

## Alternatives Considered

- Implement backend service route ownership behavior immediately.
...
Backend Service Route Ownership is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow source-first inspection map lets agents reason about future architecture without changing the running server.
...
Agents can now inspect or reason about Pitaya-aligned backend service route ownership vocabulary without treating that vocabulary as live runtime behavior.

The repository remains single-process and source-first for this concern.

## Reversal Conditions

Revisit this decision if a later ADR selects a concrete backend service route ownership, distributed runtime, service discovery, RPC, frontend/backend, event bus, protocol, persistence, hosted, or direct compatibility implementation model.
