# ADR-0212: Pitaya-Aligned Service Export Source-First Map
...
- `conversations/2026-06-13-implement-pitaya-aligned-service-export-source-first-map.md`
...
Implement `node tools/vibit inspect pitaya-service-export --json` and register `runtime.pitaya_aligned_service_export_source_first_map` as the source-first map check rule for `M-232/W-0304`.

This decision records `W-0304` as completed, with `ADR-0212` as the decision and `runtime.pitaya_aligned_service_export_source_first_map` as the check rule.

This decision does not add service export behavior, component service registration, backend service export behavior, runtime behavior, protocol messages or routes, Protobuf source, generated output, repository interfaces, PostgreSQL adapters, migrations, dependencies, authentication/session behavior changes, hosted deployments, SDK publication, release artifacts, distributed runtime behavior, or direct Nakama/Pitaya API compatibility.

## Alternatives Considered

- Implement service export behavior immediately.
...
Service Export is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow source-first inspection map lets agents reason about future architecture without changing the running server.
...
Agents can now inspect or reason about Pitaya-aligned service export vocabulary without treating that vocabulary as live runtime behavior.

The repository remains single-process and source-first for this concern.

## Reversal Conditions

Revisit this decision if a later ADR selects a concrete service export, distributed runtime, service discovery, RPC, frontend/backend, event bus, protocol, persistence, hosted, or direct compatibility implementation model.
