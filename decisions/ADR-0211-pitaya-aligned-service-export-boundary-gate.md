# ADR-0211: Pitaya-Aligned Service Export Boundary Gate
...
- `conversations/2026-06-13-define-pitaya-aligned-service-export-boundary-gate.md`
...
Define `docs/pitaya-aligned-service-export-boundary-gate.md` and register `runtime.pitaya_aligned_service_export_boundary_gate` as the gate-only repository check rule for `M-231/W-0303`.

This decision records `W-0303` as completed, with `ADR-0211` as the decision and `runtime.pitaya_aligned_service_export_boundary_gate` as the check rule.
...
Service Export is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow gate lets agents reason about future architecture without changing the running server.
