# ADR-0216: Pitaya-Aligned Frontend Message Forwarding Source-First Map
...
- `conversations/2026-06-13-implement-pitaya-aligned-frontend-message-forwarding-source-first-map.md`
...
Implement `node tools/vibit inspect pitaya-frontend-forwarding --json` and register `runtime.pitaya_aligned_frontend_message_forwarding_source_first_map` as the source-first map check rule for `M-236/W-0308`.

This decision records `W-0308` as completed, with `ADR-0216` as the decision and `runtime.pitaya_aligned_frontend_message_forwarding_source_first_map` as the check rule.
...
Frontend Message Forwarding is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow source-first inspection map lets agents reason about future architecture without changing the running server.
