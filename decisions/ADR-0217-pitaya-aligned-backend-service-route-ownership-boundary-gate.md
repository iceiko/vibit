# ADR-0217: Pitaya-Aligned Backend Service Route Ownership Boundary Gate
...
- `changes/2026-06-13-define-pitaya-aligned-backend-service-route-ownership-boundary-gate/`

Related conversations:

- `conversations/2026-06-13-define-pitaya-aligned-backend-service-route-ownership-boundary-gate.md`
...
Define `docs/pitaya-aligned-backend-service-route-ownership-boundary-gate.md` and register `runtime.pitaya_aligned_backend_service_route_ownership_boundary_gate` as the gate-only repository check rule for `M-237/W-0309`.

This decision records `W-0309` as completed, with `ADR-0217` as the decision and `runtime.pitaya_aligned_backend_service_route_ownership_boundary_gate` as the check rule.
...
Backend Service Route Ownership is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow gate lets agents reason about future architecture without changing the running server.
