# ADR-0220: Pitaya-Aligned Cluster Event Bus Source-First Map
...
- `changes/2026-06-13-implement-pitaya-aligned-cluster-event-bus-source-first-map/`

Related conversations:

- `conversations/2026-06-13-implement-pitaya-aligned-cluster-event-bus-source-first-map.md`

Related artifacts:

- `docs/pitaya-aligned-cluster-event-bus-boundary-gate.md`
- `docs/pitaya-aligned-cluster-event-bus-boundary-gate.zh-CN.md`
...
Implement `node tools/vibit inspect pitaya-cluster-event-bus --json` and register `runtime.pitaya_aligned_cluster_event_bus_source_first_map` as the source-first map check rule for `M-240/W-0312`.

This decision records `W-0312` as completed, with `ADR-0220` as the decision and `runtime.pitaya_aligned_cluster_event_bus_source_first_map` as the check rule.

This decision does not add cluster event bus behavior, outbox event delivery behavior, cluster pub/sub behavior, node event fanout behavior, runtime behavior, protocol messages or routes, Protobuf source, generated output, repository interfaces, PostgreSQL adapters, migrations, dependencies, authentication/session behavior changes, hosted deployments, SDK publication, release artifacts, distributed runtime behavior, or direct Nakama/Pitaya API compatibility.

## Alternatives Considered

- Implement cluster event bus behavior immediately.
...
Cluster Event Bus is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or RPC behavior. A narrow source-first inspection map lets agents reason about future architecture without changing the running server.
...
Agents can now inspect or reason about Pitaya-aligned cluster event bus vocabulary without treating that vocabulary as live runtime behavior.

The repository remains single-process and source-first for this concern.

## Reversal Conditions

Revisit this decision if a later ADR selects a concrete cluster event bus, distributed runtime, service discovery, RPC, frontend/backend, event bus, protocol, persistence, hosted, or direct compatibility implementation model.
