# ADR-0232: Pitaya-Aligned Remote Timeout Retry Source-First Map
...
- `changes/2026-06-14-implement-pitaya-aligned-remote-timeout-retry-source-first-map/`

Related conversations:

- `conversations/2026-06-14-implement-pitaya-aligned-remote-timeout-retry-source-first-map.md`
...
Implement `node tools/vibit inspect pitaya-remote-timeout-retry --json` and register `runtime.pitaya_aligned_remote_timeout_retry_source_first_map` as the source-first map check rule for `M-252/W-0324`.

This decision records `W-0324` as completed, with `ADR-0232` as the decision and `runtime.pitaya_aligned_remote_timeout_retry_source_first_map` as the check rule.
...
Remote Timeout Retry is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or operations behavior. A narrow source-first inspection map lets agents reason about future architecture without changing the running server.
