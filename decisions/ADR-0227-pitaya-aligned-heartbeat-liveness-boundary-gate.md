# ADR-0227: Pitaya-Aligned Heartbeat And Liveness Boundary Gate
...
- `changes/2026-06-14-define-pitaya-aligned-heartbeat-liveness-boundary-gate/`

Related conversations:

- `conversations/2026-06-14-define-pitaya-aligned-heartbeat-liveness-boundary-gate.md`
...
Define `docs/pitaya-aligned-heartbeat-liveness-boundary-gate.md` and register `runtime.pitaya_aligned_heartbeat_liveness_boundary_gate` as the gate-only repository check rule for `M-247/W-0319`.

This decision records `W-0319` as completed, with `ADR-0227` as the decision and `runtime.pitaya_aligned_heartbeat_liveness_boundary_gate` as the check rule.
...
Heartbeat And Liveness is useful Pitaya-class vocabulary, but it can easily imply distributed runtime or operations behavior. A narrow gate lets agents reason about future architecture without changing the running server.
