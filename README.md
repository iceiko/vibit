...
```text
W-0290 Implement Pitaya-aligned startup and shutdown hook source-first map
```

Use `.arch/work-items.yaml` as the source of truth for continuation. `W-0231` through `W-0242` completed the friends relationship lifecycle, persistence, repository, PostgreSQL adapter, runtime behavior, protocol route, and local proof path. `W-0243` through `W-0278` completed the post-friends route capability selection, minimum operations inspection gate, source-first operations inspection, Pitaya-aligned distributed runtime vocabulary gate, `node tools/vibit inspect pitaya-vocabulary --json`, the frontend/backend role boundary gate, `node tools/vibit inspect pitaya-roles --json`, the Pitaya-aligned server-to-server RPC boundary gate, `node tools/vibit inspect pitaya-rpc --json`, the Pitaya-aligned service discovery boundary gate, `node tools/vibit inspect pitaya-discovery --json`, the Pitaya-aligned distributed group and broadcast boundary gate, `node tools/vibit inspect pitaya-groups --json`, the Pitaya-aligned cluster-safe session routing boundary gate, `node tools/vibit inspect pitaya-sessions --json`, the next Pitaya-aligned direction selection, the Pitaya-aligned route handler pipeline boundary gate, `node tools/vibit inspect pitaya-routes --json`, the next Pitaya-aligned direction selection after the route handler pipeline map, the Pitaya-aligned serializer and message forwarding boundary gate, `node tools/vibit inspect pitaya-serializer-forwarding --json`, the next Pitaya-aligned direction selection after the serializer and message forwarding map, the Pitaya-aligned acceptor and connection lifecycle boundary gate, `node tools/vibit inspect pitaya-acceptor-connection --json`, the next Pitaya-aligned direction selection after the acceptor and connection lifecycle map, the session binding, kick/disconnect, and session data boundary gate, `node tools/vibit inspect pitaya-session-lifecycle --json`, the next Pitaya-aligned direction selection after the session lifecycle map, the Pitaya-aligned runtime observability boundary gate, `node tools/vibit inspect pitaya-observability --json`, the next Pitaya-aligned direction selection after the runtime observability map, the Pitaya-aligned metrics and tracing boundary gate, `node tools/vibit inspect pitaya-metrics-tracing --json`, the next Pitaya-aligned direction selection after the metrics and tracing map, the Pitaya-aligned dashboard and admin operations boundary gate, `node tools/vibit inspect pitaya-dashboard-admin --json`, the Pitaya-aligned runtime component lifecycle boundary gate, and `node tools/vibit inspect pitaya-component-lifecycle --json`. `ADR-0192` registered `runtime.pitaya_aligned_handler_module_registration_source_first_map`, implemented `node tools/vibit inspect pitaya-handler-modules --json`, and completed `W-0284 Implement Pitaya-aligned handler module registration source-first map`; `ADR-0193` registered `runtime.next_pitaya_aligned_direction_after_handler_module_registration_map`, completed `W-0285`, selected `define_pitaya_aligned_component_discovery_module_loading_boundary_gate`, and opened `W-0286 Define Pitaya-aligned component discovery and module loading boundary gate`; `ADR-0194` registered `runtime.pitaya_aligned_component_discovery_module_loading_boundary_gate`, completed `W-0286`, and opened `W-0287 Implement Pitaya-aligned component discovery and module loading source-first map`; `ADR-0195` registered `runtime.pitaya_aligned_component_discovery_module_loading_source_first_map`, implemented `node tools/vibit inspect pitaya-component-loading --json`, completed `W-0287`, and opened `W-0288 Select next Pitaya-aligned direction after component discovery and module loading map`; `ADR-0196` registered `runtime.next_pitaya_aligned_direction_after_component_discovery_module_loading_map`, completed `W-0288`, selected `define_pitaya_aligned_startup_shutdown_hook_boundary_gate`, and opened `W-0289 Define Pitaya-aligned startup and shutdown hook boundary gate`; `ADR-0197` registered `runtime.pitaya_aligned_startup_shutdown_hook_boundary_gate`, completed `W-0289`, and opened `W-0290 Implement Pitaya-aligned startup and shutdown hook source-first map`. The next direction is `implement_pitaya_aligned_startup_shutdown_hook_source_first_map`. A request such as `continue` or `继续推进` means: advance one `next_ready` work item unless blocked by an ask-first boundary, verification failure, or required maintainer confirmation.
...
`ADR-0189` registered `runtime.pitaya_aligned_runtime_component_lifecycle_source_first_map`, implemented `node tools/vibit inspect pitaya-component-lifecycle --json`, and completed `W-0281 Implement Pitaya-aligned runtime component lifecycle source-first map`; `ADR-0190` registered `runtime.next_pitaya_aligned_direction_after_runtime_component_lifecycle_map`, completed `W-0282 Select next Pitaya-aligned direction after runtime component lifecycle map`, and selected `define_pitaya_aligned_handler_module_registration_boundary_gate`; `ADR-0191` registered `runtime.pitaya_aligned_handler_module_registration_boundary_gate`, completed `W-0283 Define Pitaya-aligned handler module registration boundary gate`, and opened `W-0284 Implement Pitaya-aligned handler module registration source-first map`; `ADR-0192` registered `runtime.pitaya_aligned_handler_module_registration_source_first_map`, implemented `node tools/vibit inspect pitaya-handler-modules --json`, and completed `W-0284`; `ADR-0193` registered `runtime.next_pitaya_aligned_direction_after_handler_module_registration_map`, completed `W-0285`, selected `define_pitaya_aligned_component_discovery_module_loading_boundary_gate`, and opened `W-0286 Define Pitaya-aligned component discovery and module loading boundary gate`; `ADR-0194` registered `runtime.pitaya_aligned_component_discovery_module_loading_boundary_gate`, completed `W-0286`, and opened `W-0287 Implement Pitaya-aligned component discovery and module loading source-first map`; `ADR-0195` registered `runtime.pitaya_aligned_component_discovery_module_loading_source_first_map`, implemented `node tools/vibit inspect pitaya-component-loading --json`, completed `W-0287`, and opened `W-0288 Select next Pitaya-aligned direction after component discovery and module loading map`; `ADR-0196` registered `runtime.next_pitaya_aligned_direction_after_component_discovery_module_loading_map`, completed `W-0288`, and opened `W-0289 Define Pitaya-aligned startup and shutdown hook boundary gate` as next-ready; `ADR-0197` registered `runtime.pitaya_aligned_startup_shutdown_hook_boundary_gate`, completed `W-0289`, and opened `W-0290 Implement Pitaya-aligned startup and shutdown hook source-first map` as next-ready.

The next direction is `implement_pitaya_aligned_startup_shutdown_hook_source_first_map`.
