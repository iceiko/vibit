...
这条路径对应 Nakama 的 developer-experience 压力：backend framework 应该让开发者容易理解 client 如何执行 authentication、protected gameplay requests、player storage、presence、logout 和 failure behavior。vibit 采纳这种产品压力，但不复制 Nakama 的 public routes、payloads、SDK shapes、token semantics、runtime APIs 或 compatibility promises。
...
- 添加 stream subscriptions、chat rooms、groups、broadcast fanout、delivery guarantees、matchmaking、match runtime、operations/admin behavior 或 distributed runtime；
