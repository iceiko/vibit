...
- Adds the storage objects PostgreSQL adapter implementation under `runtime/internal/platform/persistence/postgres`.
- Keeps SQL mapping in the platform persistence package.
- Preserves `runtime/internal/modules/storage` as the owner of storage-neutral repository vocabulary and error types.
...
The adapter implementation adds no storage object runtime handlers, services, startup wiring, route policy, WebSocket behavior, or protocol dispatch.
...
No migration is added or changed. The adapter targets the existing `storage_objects` table.
...
- Runtime storage object behavior remains deferred, so no public route can use the adapter yet.
- Future runtime behavior must still define request identity, permission, conflict, and route-policy rules before use.
