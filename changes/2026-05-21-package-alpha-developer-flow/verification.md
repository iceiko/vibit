...
- `node tools/vibit inspect next` passed and reports `M-118/W-0190 Define release publishing decision gate` as next ready.
- `node tools/vibit check change package-alpha-developer-flow --json` passed.
...
- `node tools/vibit check schemas --json` passed after this change spec was marked verified.
