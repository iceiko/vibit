...
node tools/vibit inspect rule runtime.next_alpha_direction_after_storage_objects_local_proof
node tools/vibit check change confirm-next-alpha-direction-after-storage-objects-local-proof --json
...
- `node tools/vibit inspect next --json` passed and reported `M-142/W-0214 Define first server push and realtime messaging gate` as next ready.
- `node tools/vibit inspect rule runtime.next_alpha_direction_after_storage_objects_local_proof` passed.
- `node tools/vibit check change confirm-next-alpha-direction-after-storage-objects-local-proof --json` passed.
