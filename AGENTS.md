...
Use `.arch/reference.yaml` as the machine-readable intake point for reference alignment. Nakama is the primary product capability reference for broad game backend product capability surface. Pitaya is deferred as a future architecture reference for distributed runtime concerns and must not drive near-term cluster/RPC/frontend-backend work before a later ADR reactivates it. Preserve vibit's Agent-Native constraints when adapting reference patterns, and record why a reference pattern is adopted, adapted, or rejected.
...
`M-184/W-0256 Define Pitaya-aligned cluster-safe session routing boundary gate` is completed. It accepted `ADR-0164`, registered `runtime.pitaya_aligned_cluster_safe_session_routing_boundary_gate`, defined the gate-only cluster-safe session routing vocabulary boundary, and opened `M-185/W-0257 Implement Pitaya-aligned cluster-safe session routing source-first map` as next-ready. Keep W-0257 source-first; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, service registries, service selectors, node identity, distributed groups, group membership registries, stream subscriptions, broadcast fanout, room broadcast fanout, delivery guarantees, cluster-safe session routing behavior, session location registries, connection owner node registries, routing epoch behavior, session route targets, remote connection handoff, or distributed session routing until later bounded work items authorize them.
