...
After `ADR-0155`, the near-term priority is `M-176/W-0248 Define Pitaya-aligned frontend/backend role boundary gate`. Keep that work gate-only; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, distributed groups, broadcast fanout, or cluster-safe session routing until later bounded work items authorize them.
