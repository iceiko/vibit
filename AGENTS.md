...
After `ADR-0157`, the near-term priority is `M-178/W-0250 Define Pitaya-aligned server-to-server RPC boundary gate`. Keep that work gate-only; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, distributed groups, broadcast fanout, or cluster-safe session routing until later bounded work items authorize them.
