...
`M-182/W-0254 Define Pitaya-aligned distributed group and broadcast boundary gate` is completed. It accepted `ADR-0162`, registered `runtime.pitaya_aligned_distributed_group_broadcast_boundary_gate`, defined the gate-only `distributed_group`, `room_broadcast`, `broadcast_target`, `group_membership`, and `broadcast_fanout` vocabulary boundary, and opened `M-183/W-0255 Implement Pitaya-aligned distributed group and broadcast source-first map` as next-ready. Keep W-0255 source-first and inspection-only; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, service registries, service selectors, node identity, distributed groups, group membership registries, stream subscriptions, broadcast fanout, room broadcast fanout, delivery guarantees, or cluster-safe session routing until later bounded work items authorize them.
