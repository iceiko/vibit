...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is pre-alpha, the target is `v0.1 alpha`, and the immediate next ready work item is `W-0188 alpha_acceptance_checklist`. Do not treat the goal document as permission to publish a release, add direct Nakama/Pitaya API compatibility, or skip `.arch/work-items.yaml`.
...
`M-116 Alpha Acceptance Checklist` is active. `W-0188` is the current next ready work item. It should add a concise alpha acceptance checklist or equivalent check covering clone, prerequisites, migrations, local configuration, local onboarding posture, login, connection binding, protected inventory, presence query, logout, status endpoints, checks, redaction, and contribution entry points. Do not publish `v0.1 alpha`, add release packaging, runtime behavior, protocol routes, Protobuf sources or generated output, migrations, dependencies, broad operations/admin behavior, authentication/session behavior changes, direct Nakama/Pitaya API compatibility, or broad product modules in that slice.
