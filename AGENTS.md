...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is source-first `v0.1.0-alpha.1` with storage objects through local proof, first server push/realtime messaging runtime, realtime protocol/WebSocket outbound delivery, Nakama-first direction selection, the agent-native feature request and test workflow, the Nakama-aligned presence/status workflow pilot, and presence/status local proof hardening completed. The Nakama-first direction decision is `ADR-0127`, and its historical check rule is `runtime.next_alpha_direction_after_realtime_outbound_delivery_slice`. The workflow standard lives at `docs/agent-native-feature-request-test-workflow.md`, its decision is `ADR-0128`, and its check rule is `runtime.agent_native_feature_request_test_workflow`. The pilot decision is `ADR-0129`, its check rule is `runtime.nakama_aligned_feature_request_workflow_pilot`, and the proof-hardening decision is `ADR-0130` with check rule `runtime.presence_status_local_proof_hardening`. The current next work item is `M-151/W-0223 Strengthen authenticated gameplay failure-path verification`. Do not treat source-first alpha authorization, prototype-ready execution plan, storage objects work, realtime outbound delivery, next-direction confirmation, workflow standard, workflow pilot, or presence proof hardening as permission to add release binaries, packages, containers, checksums, signing/provenance artifacts, hosted deployments, direct Nakama/Pitaya API compatibility, dependencies, additional storage adapters, additional SQL execution, public announcements beyond the GitHub release record, paid promotion, production memory storage behavior, persistence, delivery guarantees, broad chat/social behavior, matchmaking, match runtime, distributed runtime, or to skip `.arch/work-items.yaml`; the next slice should only strengthen existing authenticated gameplay failure-path verification.
