...
The credential record schema boundary standard is `docs/credential-record-schema-boundary.md`, with `docs/credential-record-schema-boundary.zh-CN.md` as the paired Simplified Chinese translation. `ADR-0032` records the boundary decision. It ratifies future `authentication_device_credentials` semantics for `device_credential_login` but does not authorize migrations, repositories, PostgreSQL adapters, runtime lookup, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.

The token verifier record schema boundary standard is `docs/token-verifier-record-schema-boundary.md`, with `docs/token-verifier-record-schema-boundary.zh-CN.md` as the paired Simplified Chinese translation. `ADR-0033` records the boundary decision. It ratifies future `authentication_access_tokens` semantics for opaque access-token verifier storage but does not authorize migrations, repositories, PostgreSQL adapters, runtime token issuance, validation, logout, refresh, cleanup, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.

The authentication schema migration queue standard is `docs/authentication-schema-migration-queue.md`, with `docs/authentication-schema-migration-queue.zh-CN.md` as the paired Simplified Chinese translation. `ADR-0034` records the queue decision. It orders credential migration source, token verifier migration source, authentication migration static checks, repository-interface boundary, PostgreSQL adapter boundary, and milestone closeout; it does not authorize runtime credential lookup, token issuance, token validation, logout, refresh, cleanup, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.

`M-013 Login Method And Token Format Ratification` is completed by `ADR-0031`. The active authentication-adjacent milestone is now `M-014 Credential And Token Verifier Schema Ratification`, with `W-0077 Add credential PostgreSQL migration source` as the next ready work item. This next gate may add only the ratified `authentication_device_credentials` SQL-first migration source and related manifest/check updates; it does not authorize token verifier tables, repositories, PostgreSQL adapters, runtime token validation, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.
...
Use `node tools/vibit inspect rule runtime.selected_login_token_boundary --json` when a runtime check fails on the selected `device_credential_login`, opaque access-token, explicit request proof payload, refresh-token deferral, generated authentication shape deferral, authentication Protobuf deferral, WebSocket carrier deferral, schema-gate, migration, repository, adapter, or dependency boundary.
