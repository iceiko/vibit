...
`ADR-0037` closes the runtime authentication implementation boundary planning milestone and opens the generated authentication contract shape timing gate. `ADR-0038` completes the timing decision. `W-0089` completes generator/check support plus metadata-only generated authentication shape output. `ADR-0039` and `W-0090` complete the service-interface boundary step. `ADR-0040` and `W-0091` complete the verifier algorithm/redaction step. `ADR-0041` and `W-0092` complete the secret configuration/verifier key loading preparation step.
