...
`M-014 Credential And Token Verifier Schema Ratification` is completed by `ADR-0035`. The active authentication-adjacent milestone is now `M-015 Authentication PostgreSQL Adapter Implementation`. `W-0084 Implement authentication PostgreSQL adapter` is completed, and `W-0085 Close authentication PostgreSQL adapter implementation milestone` is the next ready work item. The implemented adapter is persistence-adapter-only; it does not authorize runtime token validation, token issuance, logout execution, refresh, cleanup jobs, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.
