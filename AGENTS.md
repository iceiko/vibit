...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is pre-alpha, the target is `v0.1 alpha`, and the immediate next ready work item remains `W-0178 define_presence_protocol_query_functional_slice`. Do not treat the goal document as permission to publish a release, add direct Nakama/Pitaya API compatibility, or skip `.arch/work-items.yaml`.
