...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is source-first `v0.1.0-alpha.1` with storage objects through local proof, first server push/realtime messaging runtime, realtime protocol/WebSocket outbound delivery, Nakama-first direction selection, the agent-native feature request and test workflow, the Nakama-aligned presence/status workflow pilot, presence/status local proof hardening, authenticated gameplay failure-path verification, next Nakama prototype-ready capability selection, local alpha example client path gate, local alpha example client path implementation, agent-native feature request scaffolding selection, agent-native feature request scaffolding gate, and agent-native feature request scaffolding implementation completed. The Nakama-first direction decision is `ADR-0127`, and its historical check rule is `runtime.next_alpha_direction_after_realtime_outbound_delivery_slice`. The workflow standard lives at `docs/agent-native-feature-request-test-workflow.md`, its decision is `ADR-0128`, and its check rule is `runtime.agent_native_feature_request_test_workflow`. The pilot decision is `ADR-0129`, its check rule is `runtime.nakama_aligned_feature_request_workflow_pilot`; the proof-hardening decision is `ADR-0130` with check rule `runtime.presence_status_local_proof_hardening`; the failure-path verification decision is `ADR-0131` with check rule `runtime.authenticated_gameplay_failure_path_verification`; the next capability selection decision is `ADR-0132` with check rule `runtime.next_nakama_prototype_ready_capability_selection`; the example client path gate decision is `ADR-0133` with check rule `runtime.local_alpha_example_client_path_gate`; the example client path implementation decision is `ADR-0134` with check rule `runtime.local_alpha_example_client_path_implementation`; the feature request scaffolding selection decision is `ADR-0135` with check rule `runtime.agent_native_feature_request_scaffolding_selection`; the feature request scaffolding gate decision is `ADR-0136` with check rule `runtime.agent_native_feature_request_scaffolding_gate`; and the feature request scaffolding implementation decision is `ADR-0137` with check rule `runtime.agent_native_feature_request_scaffolding_implementation`. The current next work item is `M-158/W-0230 Pilot scaffolded Nakama feature request intake`. Do not treat source-first alpha authorization, prototype-ready execution plan, storage objects work, realtime outbound delivery, next-direction confirmation, workflow standard, workflow pilot, presence proof hardening, authenticated failure-path verification, next capability selection, example client path gate, example client path implementation, feature request scaffolding selection, feature request scaffolding gate, or feature request scaffolding implementation as permission to add release binaries, packages, containers, checksums, signing/provenance artifacts, hosted deployments, direct Nakama/Pitaya API compatibility, dependencies, additional storage adapters, additional SQL execution, public announcements beyond the GitHub release record, paid promotion, production memory storage behavior, persistence, delivery guarantees, broad chat/social behavior, matchmaking, match runtime, distributed runtime, SDK publication, generated client libraries, runtime scaffolding, or to skip `.arch/work-items.yaml`; the next slice should only use the source-first scaffold for a bounded Nakama feature request intake and choose a later authorized follow-up.
...
Future major work must map to a roadmap family: identity/auth/session, connection lifecycle, storage, presence/status/notifications, chat/realtime messaging, friends/groups/parties, leaderboards/tournaments, economy/progression, matchmaking, match runtime, server runtime hooks/RPC, operations, SDK/developer experience, distributed runtime, or agent-native requirement/test workflow. The near-term priority is now `M-158/W-0230 Pilot scaffolded Nakama feature request intake`. This next slice must follow the workflow from `docs/agent-native-feature-request-test-workflow.md`, the Nakama-first decision in `ADR-0127`, the example client path implementation in `ADR-0134`, the scaffolding selection in `ADR-0135`, the scaffolding gate in `ADR-0136`, and the scaffolding implementation in `ADR-0137`; use `tools/vibit scaffold feature` on one bounded Nakama-style request intake, keep Pitaya deferred, and do not jump to startup wiring, broad chat/social modules, matchmaking, match runtime, SDK publication, hosted demos, distributed runtime, public announcements, hosted deployments, release artifacts, broad new route families, persistence, delivery guarantees, stream subscriptions, chat rooms, groups, broadcast fanout, generated client libraries, or direct Nakama/Pitaya API compatibility before a later bounded work item authorizes it.
