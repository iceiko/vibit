...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is source-first `v0.1.0-alpha.1` with storage objects through local proof, first server push/realtime messaging runtime, realtime protocol/WebSocket outbound delivery, Nakama-first direction selection, the agent-native feature request and test workflow, the Nakama-aligned presence/status workflow pilot, presence/status local proof hardening, authenticated gameplay failure-path verification, next Nakama prototype-ready capability selection, local alpha example client path gate, local alpha example client path implementation, agent-native feature request scaffolding selection, agent-native feature request scaffolding gate, agent-native feature request scaffolding implementation, and the scaffolded Nakama feature request intake pilot completed. The Nakama-first direction decision is `ADR-0127`, and its historical check rule is `runtime.next_alpha_direction_after_realtime_outbound_delivery_slice`. The workflow standard lives at `docs/agent-native-feature-request-test-workflow.md`, its decision is `ADR-0128`, and its check rule is `runtime.agent_native_feature_request_test_workflow`. The pilot decision is `ADR-0129`, its check rule is `runtime.nakama_aligned_feature_request_workflow_pilot`; the proof-hardening decision is `ADR-0130` with check rule `runtime.presence_status_local_proof_hardening`; the failure-path verification decision is `ADR-0131` with check rule `runtime.authenticated_gameplay_failure_path_verification`; the next capability selection decision is `ADR-0132` with check rule `runtime.next_nakama_prototype_ready_capability_selection`; the example client path gate decision is `ADR-0133` with check rule `runtime.local_alpha_example_client_path_gate`; the example client path implementation decision is `ADR-0134` with check rule `runtime.local_alpha_example_client_path_implementation`; the feature request scaffolding selection decision is `ADR-0135` with check rule `runtime.agent_native_feature_request_scaffolding_selection`; the feature request scaffolding gate decision is `ADR-0136` with check rule `runtime.agent_native_feature_request_scaffolding_gate`; the feature request scaffolding implementation decision is `ADR-0137` with check rule `runtime.agent_native_feature_request_scaffolding_implementation`; and the scaffolded Nakama feature request intake pilot decision is `ADR-0138` with check rule `runtime.scaffolded_nakama_feature_request_intake_pilot`. The current next work item is `M-159/W-0231 Define friends relationship lifecycle gate`. Do not treat source-first alpha authorization, prototype-ready execution plan, storage objects work, realtime outbound delivery, next-direction confirmation, workflow standard, workflow pilot, presence proof hardening, authenticated failure-path verification, next capability selection, example client path gate, example client path implementation, feature request scaffolding selection, feature request scaffolding gate, feature request scaffolding implementation, or scaffolded Nakama intake as permission to add release binaries, packages, containers, checksums, signing/provenance artifacts, hosted deployments, direct Nakama/Pitaya API compatibility, dependencies, additional storage adapters, additional SQL execution, public announcements beyond the GitHub release record, paid promotion, production memory storage behavior, persistence, delivery guarantees, broad chat/social behavior, matchmaking, match runtime, distributed runtime, SDK publication, generated client libraries, runtime scaffolding, friend runtime behavior, protocol routes, Protobuf sources, generated output, migrations, or to skip `.arch/work-items.yaml`; the next slice should only define the friends relationship lifecycle gate for the selected `friends_groups_and_parties` capability.
...
Future major work must map to a roadmap family: identity/auth/session, connection lifecycle, storage, presence/status/notifications, chat/realtime messaging, friends/groups/parties, leaderboards/tournaments, economy/progression, matchmaking, match runtime, server runtime hooks/RPC, operations, SDK/developer experience, distributed runtime, or agent-native requirement/test workflow. The near-term priority is now `M-159/W-0231 Define friends relationship lifecycle gate`. This next slice must follow the workflow from `docs/agent-native-feature-request-test-workflow.md`, the Nakama-first decision in `ADR-0127`, the scaffolding implementation in `ADR-0137`, and the scaffolded intake pilot in `ADR-0138`; define the `friends_groups_and_parties` relationship lifecycle gate only, keep Pitaya deferred, and do not jump to startup wiring, friend runtime behavior, broad chat/social modules, matchmaking, match runtime, SDK publication, hosted demos, distributed runtime, public announcements, hosted deployments, release artifacts, broad new route families, persistence, delivery guarantees, stream subscriptions, chat rooms, groups, parties, broadcast fanout, generated client libraries, Protobuf sources, generated output, migrations, or direct Nakama/Pitaya API compatibility before a later bounded work item authorizes it.
