...
Future major work must map to a roadmap family: identity/auth/session, connection lifecycle, storage, presence/status/notifications, chat/realtime messaging, friends/groups/parties, leaderboards/tournaments, economy/progression, matchmaking, match runtime, server runtime hooks/RPC, operations, SDK/developer experience, distributed runtime, or agent-native requirement/test workflow. The near-term priority is now `M-164/W-0236 Define friends relationship PostgreSQL adapter gate`. This next slice must follow the workflow from `docs/agent-native-feature-request-test-workflow.md`, the Nakama-first decision in `ADR-0127`, the scaffolding implementation in `ADR-0137`, the scaffolded intake pilot in `ADR-0138`, the friends relationship lifecycle gate in `ADR-0139`, the friends relationship persistence schema gate in `ADR-0140`, the friends relationship migration source in `ADR-0141`, the friends relationship repository boundary in `ADR-0142`, and the friends relationship repository interface implementation in `ADR-0143`; define only the friends relationship PostgreSQL adapter gate, keep Pitaya deferred, and do not jump to PostgreSQL adapter implementation, startup wiring, friend runtime behavior, broad chat/social modules, matchmaking, match runtime, SDK publication, hosted demos, distributed runtime, public announcements, hosted deployments, release artifacts, broad new route families, delivery guarantees, stream subscriptions, chat rooms, groups, parties, broadcast fanout, generated client libraries, Protobuf sources, generated output, event/audit tables, or direct Nakama/Pitaya API compatibility before a later bounded work item authorizes it.
