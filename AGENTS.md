...
After `ADR-0159`, the near-term priority is `M-180/W-0252 Define Pitaya-aligned service discovery boundary gate`. Keep that work gate-only; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, distributed groups, broadcast fanout, or cluster-safe session routing until later bounded work items authorize them.
