...
Use `.arch/reference.yaml` as the machine-readable intake point for reference alignment. Nakama is the primary product capability reference for broad game backend product capability surface. Pitaya is the active architecture reference for source-first distributed runtime vocabulary, role, RPC, discovery, group/broadcast, and session-routing maps. Pitaya alignment must remain boundary-first and inspection-only until later bounded work items authorize implementation behavior. Preserve vibit's Agent-Native constraints when adapting reference patterns, and record why a reference pattern is adopted, adapted, or rejected.
...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is source-first `v0.1.0-alpha.1` with storage objects through local proof, first server push/realtime messaging runtime, realtime protocol/WebSocket outbound delivery, Nakama-first direction selection, the agent-native feature request and test workflow, the Nakama-aligned presence/status workflow pilot, presence/status local proof hardening, authenticated gameplay failure-path verification, next Nakama prototype-ready capability selection, local alpha example client path gate, local alpha example client path implementation, agent-native feature request scaffolding selection, agent-native feature request scaffolding gate, agent-native feature request scaffolding implementation, the scaffolded Nakama feature request intake pilot, the friends relationship lifecycle gate, the friends relationship persistence schema gate, the friends relationship migration source, the friends relationship repository boundary, the friends relationship repository interface implementation, the friends relationship PostgreSQL adapter gate, the friends relationship PostgreSQL adapter implementation, the friends relationship runtime behavior gate, the friends relationship runtime behavior implementation, and the friends relationship protocol route gate completed. The Nakama-first direction decision is `ADR-0127`, and its historical check rule is `runtime.next_alpha_direction_after_realtime_outbound_delivery_slice`. The workflow standard lives at `docs/agent-native-feature-request-test-workflow.md`, its decision is `ADR-0128`, and its check rule is `runtime.agent_native_feature_request_test_workflow`. The pilot decision is `ADR-0129`, its check rule is `runtime.nakama_aligned_feature_request_workflow_pilot`; the proof-hardening decision is `ADR-0130` with check rule `runtime.presence_status_local_proof_hardening`; the failure-path verification decision is `ADR-0131` with check rule `runtime.authenticated_gameplay_failure_path_verification`; the next capability selection decision is `ADR-0132` with check rule `runtime.next_nakama_prototype_ready_capability_selection`; the example client path gate decision is `ADR-0133` with check rule `runtime.local_alpha_example_client_path_gate`; the example client path implementation decision is `ADR-0134` with check rule `runtime.local_alpha_example_client_path_implementation`; the feature request scaffolding selection decision is `ADR-0135` with check rule `runtime.agent_native_feature_request_scaffolding_selection`; the feature request scaffolding gate decision is `ADR-0136` with check rule `runtime.agent_native_feature_request_scaffolding_gate`; the feature request scaffolding implementation decision is `ADR-0137` with check rule `runtime.agent_native_feature_request_scaffolding_implementation`; the scaffolded Nakama feature request intake pilot decision is `ADR-0138` with check rule `runtime.scaffolded_nakama_feature_request_intake_pilot`; the friends relationship lifecycle gate decision is `ADR-0139` with check rule `runtime.friends_relationship_lifecycle_gate`; the friends relationship persistence schema gate decision is `ADR-0140` with check rule `runtime.friends_relationship_persistence_schema_gate`; the friends relationship migration source decision is `ADR-0141` with check rule `runtime.friends_relationship_migration_source`; the friends relationship repository boundary decision is `ADR-0142` with check rule `runtime.friends_relationship_repository_boundary`; the friends relationship repository interface implementation decision is `ADR-0143` with check rule `runtime.friends_relationship_repository_interface_implementation`; the friends relationship PostgreSQL adapter gate decision is `ADR-0144` with check rule `runtime.friends_relationship_postgresql_adapter_gate`; the friends relationship PostgreSQL adapter implementation decision is `ADR-0145` with check rule `runtime.friends_relationship_postgresql_adapter_implementation`; the friends relationship runtime behavior gate decision is `ADR-0146` with check rule `runtime.friends_relationship_runtime_behavior_gate`; the friends relationship runtime behavior implementation decision is `ADR-0147` with check rule `runtime.friends_relationship_runtime_behavior_implementation`; and the friends relationship protocol route gate decision is `ADR-0148` with check rule `runtime.friends_relationship_protocol_route_gate`. At that point, the next work item was `M-169/W-0241 Implement friends relationship protocol route`. Do not treat source-first alpha authorization, prototype-ready execution plan, storage objects work, realtime outbound delivery, next-direction confirmation, workflow standard, workflow pilot, presence proof hardening, authenticated failure-path verification, next capability selection, example client path gate, example client path implementation, feature request scaffolding selection, feature request scaffolding gate, feature request scaffolding implementation, scaffolded Nakama intake, friends relationship lifecycle gate, friends relationship persistence schema gate, friends relationship migration source, friends relationship repository boundary, friends relationship repository interface implementation, friends relationship PostgreSQL adapter gate, friends relationship PostgreSQL adapter implementation, friends relationship runtime behavior gate, friends relationship runtime behavior implementation, or friends relationship protocol route gate as permission to add release binaries, packages, containers, checksums, signing/provenance artifacts, hosted deployments, direct Nakama/Pitaya API compatibility, dependencies, additional storage adapters beyond the bounded friends relationship adapter, additional SQL execution beyond the existing migration sources and bounded adapter scope, public announcements beyond the GitHub release record, paid promotion, production memory storage behavior, broad chat/social behavior, matchmaking, match runtime, distributed runtime, SDK publication, generated client libraries, runtime scaffolding beyond the bounded friends runtime service slice, protocol route implementation, Protobuf sources, generated output, event/audit tables, or to skip `.arch/work-items.yaml`; that slice should implement only the friends relationship protocol route for the selected `friends_groups_and_parties` capability.

`M-170/W-0242 Prove friends relationship protocol route in local alpha request flow` is completed after `M-169/W-0241` registered `runtime.friends_relationship_protocol_route_implementation`. It accepted `ADR-0150`, registered `runtime.friends_relationship_protocol_route_local_proof`, and opened `M-171/W-0243 Select next Nakama prototype-ready capability after friends relationship route proof`. `M-171/W-0243` through `M-186/W-0258` are completed: `W-0243` accepted `ADR-0151` and registered `runtime.next_nakama_prototype_ready_capability_after_friends_route_proof`; `W-0244` accepted `ADR-0152` and registered `runtime.minimum_operations_inspection_surface_gate`; `W-0245` accepted `ADR-0153`, registered `runtime.minimum_operations_inspection_source_first_surface_implementation`, and implemented `node tools/vibit inspect operations --json`; `W-0246` accepted `ADR-0154` and registered `runtime.pitaya_aligned_distributed_runtime_vocabulary_reactivation_gate`; `W-0247` accepted `ADR-0155`, registered `runtime.pitaya_aligned_distributed_runtime_vocabulary_source_first_map`, and implemented `node tools/vibit inspect pitaya-vocabulary --json`; `W-0248` accepted `ADR-0156` and registered `runtime.pitaya_aligned_frontend_backend_role_boundary_gate`; `W-0249` accepted `ADR-0157`, registered `runtime.pitaya_aligned_frontend_backend_role_source_first_map`, and implemented `node tools/vibit inspect pitaya-roles --json`; `W-0250` accepted `ADR-0158`, registered `runtime.pitaya_aligned_server_to_server_rpc_boundary_gate`, and defined the gate-only server-to-server RPC and remote-call vocabulary boundary; `W-0251` accepted `ADR-0159`, registered `runtime.pitaya_aligned_server_to_server_rpc_source_first_map`, and implemented `node tools/vibit inspect pitaya-rpc --json`; `W-0252` accepted `ADR-0160`, registered `runtime.pitaya_aligned_service_discovery_boundary_gate`, and defined the gate-only service discovery vocabulary boundary; `W-0253` accepted `ADR-0161`, registered `runtime.pitaya_aligned_service_discovery_source_first_map`, and implemented `node tools/vibit inspect pitaya-discovery --json`; `W-0254` accepted `ADR-0162`, registered `runtime.pitaya_aligned_distributed_group_broadcast_boundary_gate`, and defined the gate-only distributed group and broadcast vocabulary boundary; `W-0255` accepted `ADR-0163`, registered `runtime.pitaya_aligned_distributed_group_broadcast_source_first_map`, and implemented `node tools/vibit inspect pitaya-groups --json`; `W-0256` accepted `ADR-0164`, registered `runtime.pitaya_aligned_cluster_safe_session_routing_boundary_gate`, and defined the gate-only cluster-safe session routing vocabulary boundary; `W-0257` accepted `ADR-0165`, registered `runtime.pitaya_aligned_cluster_safe_session_routing_source_first_map`, and implemented `node tools/vibit inspect pitaya-sessions --json`; and `W-0258` accepted `ADR-0166`, registered `runtime.next_pitaya_aligned_direction_after_cluster_safe_session_routing_map`, and selected `define_pitaya_aligned_route_handler_pipeline_boundary_gate`. These completed work items opened `M-187/W-0259 Define Pitaya-aligned route handler pipeline boundary gate` as next-ready. The current next work item is `W-0260 Implement Pitaya-aligned route handler pipeline source-first map`; it should implement only a source-first route handler pipeline map without adding route handler implementation, handler routing behavior, pipeline middleware behavior, serializer behavior, message forwarding behavior, service discovery implementation, service registries, service selectors, node identity, server-to-server RPC implementation, remote calls, frontend/backend server role implementation, distributed runtime behavior, distributed groups, group membership registries, stream subscriptions, room broadcast fanout, broadcast delivery guarantees, cluster-safe session routing behavior, session location registries, connection owner node registries, routing epoch behavior, session route targets, remote connection handoff, distributed session routing, operations/admin endpoints, metrics endpoints, observability pipelines, dashboards, new protocol shape, repository interfaces, PostgreSQL adapters, migrations, dependencies, SDK publication, hosted deployment, or direct Nakama/Pitaya API compatibility.
...
2. Write user-visible acceptance criteria for non-trivial product behavior.
3. Write a test plan before implementation, or explicitly record why tests are not applicable.
4. Identify affected modules and contracts.
5. Write or update the change spec when the change is large enough to need durable context.
6. Update schemas, manifests, or contracts before implementation when public behavior changes.
7. Generate repeatable structure when generators exist.
8. Add or update focused tests before or alongside implementation.
9. Implement only inside the declared boundary.
10. Run relevant verification commands.
11. Update documentation and translations.
12. Record what was verified and what was not verified.
...
The product purpose is AI-native development and AI-native testing. When a user states a backend requirement, the expected direction is not only "write code"; it is for AI agents to help produce the bounded spec, acceptance criteria, test plan, tests, implementation, verification, documentation, and durable project memory.
...
Before adding new game server capability families or runtime subsystems, agents must check `.arch/reference.yaml` and `docs/reference-game-server-alignment.md`. Map the proposal to the relevant Nakama capability family, then keep the implementation sequence aligned with vibit's contract-first, manifest-first, generated, tested, and checkable architecture. Do not copy external APIs without an explicit compatibility ADR. Pitaya is deferred as a future architecture reference; do not add Pitaya-style cluster/RPC/frontend-backend/service-discovery work before a later ADR reactivates it and the modular monolith proof slice is stable.
...
`M-096 Nakama Pitaya Product Parity Roadmap` is completed. `W-0168` added `docs/nakama-pitaya-product-parity-roadmap.md`, `docs/nakama-pitaya-product-parity-roadmap.zh-CN.md`, and `ADR-0078`. `W-0219` and `ADR-0127` refine the posture: Nakama is now the primary product capability reference, Pitaya is deferred as a future architecture reference, and AI-native development/testing is the product purpose. Product parity means Nakama-style common capability coverage and comparable usefulness; it does not mean direct API compatibility. `runtime.reference_product_parity_roadmap` is the repository check rule.
...
`M-145/W-0217 Define realtime protocol and WebSocket outbound delivery gate` is completed. It accepted `ADR-0125`, registered `runtime.realtime_protocol_websocket_outbound_delivery_gate`, preserved Nakama/Pitaya alignment without direct compatibility, and opened `M-146/W-0218 Implement realtime protocol and WebSocket outbound delivery slice` as next-ready. `M-146/W-0218 Implement realtime protocol and WebSocket outbound delivery slice` is completed. It accepted `ADR-0126`, registered `runtime.realtime_protocol_websocket_outbound_delivery_implementation`, added vibit-native realtime Protobuf payloads, generated output, protocol bridge mapping, and best-effort single-process WebSocket outbound delivery while keeping startup wiring, persistence, delivery guarantees, stream subscriptions, chat, groups, broadcast fanout, distributed runtime, matchmaking, match runtime, and direct compatibility deferred.

Future major work must map to a roadmap family: identity/auth/session, connection lifecycle, storage, presence/status/notifications, chat/realtime messaging, friends/groups/parties, leaderboards/tournaments, economy/progression, matchmaking, match runtime, server runtime hooks/RPC, operations, SDK/developer experience, distributed runtime, or agent-native requirement/test workflow. The near-term priority is now `M-169/W-0241 Implement friends relationship protocol route`. This next slice must follow the workflow from `docs/agent-native-feature-request-test-workflow.md`, the Nakama-first decision in `ADR-0127`, the scaffolding implementation in `ADR-0137`, the scaffolded intake pilot in `ADR-0138`, the friends relationship lifecycle gate in `ADR-0139`, the friends relationship persistence schema gate in `ADR-0140`, the friends relationship migration source in `ADR-0141`, the friends relationship repository boundary in `ADR-0142`, the friends relationship repository interface implementation in `ADR-0143`, the friends relationship PostgreSQL adapter gate in `ADR-0144`, the friends relationship PostgreSQL adapter implementation in `ADR-0145`, the friends relationship runtime behavior gate in `ADR-0146`, the friends relationship runtime behavior implementation in `ADR-0147`, and the friends relationship protocol route gate in `ADR-0148`; implement only the friends relationship protocol route, keep Pitaya deferred, and do not jump to startup wiring beyond gate documentation, route implementation, broad chat/social modules, matchmaking, match runtime, SDK publication, hosted demos, distributed runtime, public announcements, hosted deployments, release artifacts, broad new route families, delivery guarantees, stream subscriptions, chat rooms, groups, parties, broadcast fanout, generated client libraries, Protobuf sources, generated output, event/audit tables, or direct Nakama/Pitaya API compatibility before a later bounded work item authorizes it.

`M-186/W-0258 Select next Pitaya-aligned direction after cluster-safe session routing map` is completed. It accepted `ADR-0166`, registered `runtime.next_pitaya_aligned_direction_after_cluster_safe_session_routing_map`, selected `define_pitaya_aligned_route_handler_pipeline_boundary_gate`, and opened `M-187/W-0259 Define Pitaya-aligned route handler pipeline boundary gate` as next-ready.

`M-187/W-0259 Define Pitaya-aligned route handler pipeline boundary gate` is completed. It accepted `ADR-0167`, registered `runtime.pitaya_aligned_route_handler_pipeline_boundary_gate`, defined the gate-only route handler pipeline vocabulary boundary, and opened `M-188/W-0260 Implement Pitaya-aligned route handler pipeline source-first map` as next-ready. Keep W-0260 source-first only; do not implement route handlers, handler routing behavior, handler pipeline behavior, pipeline middleware behavior, serializer behavior, message forwarding behavior, backend route targeting, frontend/backend roles, cluster/RPC/service-discovery behavior, service registries, service selectors, node identity, distributed groups, group membership registries, stream subscriptions, broadcast fanout, room broadcast fanout, delivery guarantees, cluster-safe session routing behavior, session location registries, connection owner node registries, routing epoch behavior, session route targets, remote connection handoff, or distributed session routing until later bounded work items authorize them.
