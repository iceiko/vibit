...
After `ADR-0160`, the near-term priority is `M-181/W-0253 Implement Pitaya-aligned service discovery source-first map`. Keep that work source-first; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, service registries, service selectors, node identity, distributed groups, broadcast fanout, or cluster-safe session routing until later bounded work items authorize them.
