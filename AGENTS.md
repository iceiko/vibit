...
`M-181/W-0253 Implement Pitaya-aligned service discovery source-first map` is completed. It accepted `ADR-0161`, registered `runtime.pitaya_aligned_service_discovery_source_first_map`, implemented `node tools/vibit inspect pitaya-discovery --json`, and opened `M-182/W-0254 Define Pitaya-aligned distributed group and broadcast boundary gate` as next-ready. Keep W-0254 gate-only and source-first; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, service registries, service selectors, node identity, distributed groups, broadcast fanout, room broadcast fanout, or cluster-safe session routing until later bounded work items authorize them.
