...
After `ADR-0156`, the near-term priority is `M-177/W-0249 Implement Pitaya-aligned frontend/backend role source-first map`. Keep that work source-first only; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, distributed groups, broadcast fanout, or cluster-safe session routing until later bounded work items authorize them.
