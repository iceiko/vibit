...
`M-013 Login Method And Token Format Ratification` is completed by `ADR-0031`. The active authentication-adjacent milestone is now `M-014 Credential And Token Verifier Schema Ratification`, with `W-0078 Add token verifier PostgreSQL migration source` as the next ready work item. This next gate may add only the ratified `authentication_access_tokens` SQL-first migration source and related manifest/check updates; it does not authorize repositories, PostgreSQL adapters, runtime token validation, token issuance, logout, refresh, cleanup, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.
