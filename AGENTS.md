...
After `ADR-0158`, the near-term priority is `M-179/W-0251 Implement Pitaya-aligned server-to-server RPC source-first map`. Keep that work source-first only; do not implement frontend/backend roles, cluster/RPC/service-discovery behavior, distributed groups, broadcast fanout, or cluster-safe session routing until later bounded work items authorize them.
