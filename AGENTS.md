...
`M-170/W-0242 Prove friends relationship protocol route in local alpha request flow` is completed after `M-169/W-0241` registered `runtime.friends_relationship_protocol_route_implementation`. It accepted `ADR-0150`, registered `runtime.friends_relationship_protocol_route_local_proof`, and opened `M-171/W-0243 Select next Nakama prototype-ready capability after friends relationship route proof` as next-ready. The current next work item is `W-0243 Select next Nakama prototype-ready capability after friends relationship route proof`; it should select only the next bounded prototype-ready capability direction without adding new protocol shape, service behavior, repository interfaces, PostgreSQL adapters, migrations, dependencies, event/audit tables, broad social features, SDK publication, hosted deployment, or direct Nakama/Pitaya API compatibility.
...
After `ADR-0150`, the near-term priority is `M-171/W-0243 Select next Nakama prototype-ready capability after friends relationship route proof`. Keep that work to selection only.
