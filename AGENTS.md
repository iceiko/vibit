...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is source-first `v0.1.0-alpha.1` with storage objects through local proof, first server push/realtime messaging runtime, realtime protocol/WebSocket outbound delivery, Nakama-first direction selection, and the agent-native feature request and test workflow completed. The workflow standard lives at `docs/agent-native-feature-request-test-workflow.md`, its decision is `ADR-0128`, and its check rule is `runtime.agent_native_feature_request_test_workflow`. The previous direction-selection check rule is `runtime.next_alpha_direction_after_realtime_outbound_delivery_slice`. The current next work item is `M-149/W-0221 Pilot Nakama-aligned feature request workflow`. Do not treat source-first alpha authorization, prototype-ready execution plan, storage objects work, realtime outbound delivery, next-direction confirmation, or the workflow standard as permission to add release binaries, packages, containers, checksums, signing/provenance artifacts, hosted deployments, direct Nakama/Pitaya API compatibility, dependencies, additional storage adapters, additional SQL execution, public announcements beyond the GitHub release record, paid promotion, production memory storage behavior, persistence, delivery guarantees, broad chat/social behavior, matchmaking, match runtime, distributed runtime, or to skip `.arch/work-items.yaml`; the next slice should only pilot the Nakama-aligned feature request workflow.
