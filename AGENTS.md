...
`M-170/W-0242 Prove friends relationship protocol route in local alpha request flow` is completed after `M-169/W-0241` registered `runtime.friends_relationship_protocol_route_implementation`. It accepted `ADR-0150`, registered `runtime.friends_relationship_protocol_route_local_proof`, and opened `M-171/W-0243 Select next Nakama prototype-ready capability after friends relationship route proof`. `M-171/W-0243` is completed; it accepted `ADR-0151`, registered `runtime.next_nakama_prototype_ready_capability_after_friends_route_proof`, selected `define_minimum_operations_inspection_surface_gate`, and opened `M-172/W-0244 Define minimum operations inspection surface gate` as next-ready. `M-172/W-0244` is completed; it accepted `ADR-0152`, registered `runtime.minimum_operations_inspection_surface_gate`, defined the source-first local operations inspection gate, and opened `M-173/W-0245 Implement minimum operations inspection source-first surface` as next-ready. The current next work item is `W-0245 Implement minimum operations inspection source-first surface`; it should implement only the source-first operations inspection surface without adding operations/admin endpoints, metrics endpoints, observability pipelines, dashboards, new protocol shape, service behavior, repository interfaces, PostgreSQL adapters, migrations, dependencies, event/audit tables, broad social features, SDK publication, hosted deployment, or direct Nakama/Pitaya API compatibility.
...
After `ADR-0152`, the near-term priority is `M-173/W-0245 Implement minimum operations inspection source-first surface`. Keep that work within the source-first operations inspection gate.
