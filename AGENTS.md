...
Use `docs/v0.1-alpha-goal.md` as the short-term release-target intake. The current release state is source-first `v0.1.0-alpha.1` with the first alpha user discovery loop, feedback intake surface, product maturity milestones, prototype-ready execution plan, local development path gate, local development path package, storage objects behavior gate, storage objects persistence schema gate, storage objects migration source, storage objects repository boundary, storage objects repository interface implementation, storage objects PostgreSQL adapter gate, storage objects PostgreSQL adapter implementation, storage objects runtime behavior gate, storage objects runtime behavior implementation, storage objects protocol route gate, storage objects protocol route implementation, storage objects protocol route local proof, next-direction selection after storage objects local proof, first server push/realtime messaging gate, first server push/realtime messaging runtime slice, next-direction selection after realtime runtime slice, realtime protocol and WebSocket outbound delivery gate, realtime protocol and WebSocket outbound delivery implementation, and next-direction confirmation after realtime outbound delivery slice completed. The target is `v0.1 alpha`, product maturity milestones live at `docs/product-maturity-milestones.md`, the prototype-ready execution plan lives at `docs/prototype-ready-foundation-execution-plan.md`, the local development path gate lives at `docs/prototype-ready-local-development-path-gate.md`, the local development path package lives at `docs/prototype-ready-local-development-path-package.md`, the storage objects behavior gate lives at `docs/storage-objects-behavior-gate.md`, the storage objects persistence schema gate lives at `docs/storage-objects-persistence-schema-gate.md`, the storage objects migration source is `runtime/migrations/postgres/000006_create_storage_objects.sql`, the storage objects repository boundary lives at `docs/storage-objects-repository-boundary.md`, the storage objects repository interface is `runtime/internal/modules/storage/repository.go`, the storage objects PostgreSQL adapter gate lives at `docs/storage-objects-postgresql-adapter-gate.md`, the storage objects PostgreSQL adapter implementation is `runtime/internal/platform/persistence/postgres/storage_object_repository.go` with check rule `runtime.storage_objects_postgresql_adapter_implementation`, the storage objects runtime behavior gate lives at `docs/storage-objects-runtime-behavior-gate.md` with check rule `runtime.storage_objects_runtime_behavior_gate`, the storage objects runtime behavior implementation is `runtime/internal/app/storage/service.go` with check rule `runtime.storage_objects_runtime_behavior_implementation`, the storage objects protocol route gate lives at `docs/storage-objects-protocol-route-gate.md` with check rule `runtime.storage_objects_protocol_route_gate`, the storage objects protocol route implementation is recorded in `proto/vibit/storage/v1/storage.proto`, `runtime/internal/app/bootstrap/storage.go`, `runtime/internal/platform/protocol/protobuf/storage_bridge.go`, and `decisions/ADR-0119-storage-objects-protocol-route-implementation.md` with check rule `runtime.storage_objects_protocol_route_implementation`, the storage objects protocol route local proof is recorded in `runtime/internal/platform/protocol/protobuf/authenticated_gameplay_e2e_test.go`, `examples/local-alpha-request-loop.sh`, and `decisions/ADR-0120-storage-objects-protocol-route-local-proof.md` with check rule `runtime.storage_objects_protocol_route_local_proof`, the next-direction selection after storage objects local proof is recorded in `decisions/ADR-0121-next-alpha-direction-first-server-push-realtime-messaging-gate.md` with check rule `runtime.next_alpha_direction_after_storage_objects_local_proof`, the first server push and realtime messaging gate is recorded in `docs/first-server-push-realtime-messaging-gate.md` and `decisions/ADR-0122-first-server-push-realtime-messaging-gate.md` with check rule `runtime.first_server_push_realtime_messaging_gate`, the first server push and realtime messaging runtime slice is recorded in `runtime/internal/app/realtime/service.go`, `runtime/internal/app/realtime/service_test.go`, and `decisions/ADR-0123-first-server-push-realtime-messaging-runtime-slice.md` with check rule `runtime.first_server_push_realtime_messaging_runtime_slice`, the next-direction selection after realtime runtime slice is recorded in `decisions/ADR-0124-next-alpha-direction-realtime-protocol-websocket-outbound-delivery-gate.md` with check rule `runtime.next_alpha_direction_after_realtime_runtime_slice`, the realtime protocol and WebSocket outbound delivery gate is recorded in `docs/realtime-protocol-websocket-outbound-delivery-gate.md` and `decisions/ADR-0125-realtime-protocol-websocket-outbound-delivery-gate.md` with check rule `runtime.realtime_protocol_websocket_outbound_delivery_gate`, the realtime protocol and WebSocket outbound delivery implementation is recorded in `proto/vibit/realtime/v1/realtime.proto`, `runtime/internal/platform/protocol/protobuf/realtime_bridge.go`, `runtime/internal/platform/transport/ws/outbound.go`, and `decisions/ADR-0126-realtime-protocol-websocket-outbound-delivery-implementation.md` with check rule `runtime.realtime_protocol_websocket_outbound_delivery_implementation`, `M-147/W-0219 Confirm next alpha direction after realtime outbound delivery slice` is completed by `ADR-0127` with check rule `runtime.next_alpha_direction_after_realtime_outbound_delivery_slice`, and the next work item is `M-148/W-0220 Define agent-native feature request and test workflow`. The alpha acceptance checklist lives at `docs/alpha-acceptance-checklist.md`, the packaged local alpha developer flow lives at `docs/alpha-developer-flow.md`, the release publishing decision gate lives at `docs/release-publishing-decision-gate.md`, the release execution preparation gate lives at `docs/release-execution-preparation-gate.md`, the release execution authorization gate lives at `docs/release-execution-authorization-gate.md`, the release execution maintainer decision lives at `docs/release-execution-maintainer-decision.md`, the release identifier plan lives at `docs/release-identifier-artifact-plan.md`, final authorization lives at `docs/release-execution-final-authorization.md`, the first alpha user discovery loop lives at `docs/first-alpha-user-discovery-loop.md`, and first alpha feedback intake lives at `docs/first-alpha-feedback-intake-surfaces.md`. Do not treat source-first alpha authorization, the user discovery loop, feedback intake, prototype-ready execution plan, local development path gate, local development path package, storage objects behavior gate, storage objects persistence schema gate, storage objects migration source, storage objects repository boundary, storage objects repository interface implementation, storage objects PostgreSQL adapter gate, storage objects PostgreSQL adapter implementation, storage objects runtime behavior gate, storage objects runtime behavior implementation, storage objects protocol route gate, storage objects protocol route implementation, storage objects protocol route local proof, first server push and realtime messaging gate, first server push and realtime messaging runtime slice, next-direction selection after realtime runtime slice, realtime protocol and WebSocket outbound delivery gate, realtime protocol and WebSocket outbound delivery implementation, or next-direction confirmation after realtime outbound delivery slice as permission to add release binaries, packages, containers, checksums, signing/provenance artifacts, hosted deployments, direct Nakama/Pitaya API compatibility, dependencies, additional storage adapters, additional SQL execution, public announcements beyond the GitHub release record, paid promotion, production memory storage behavior, persistence, delivery guarantees, broad chat/social behavior, matchmaking, match runtime, distributed runtime, or to skip `.arch/work-items.yaml`; the next slice should only define the agent-native feature request and test workflow.
...
Future major work must map to a roadmap family: identity/auth/session, connection lifecycle, storage, presence/status/notifications, chat/realtime messaging, friends/groups/parties, leaderboards/tournaments, economy/progression, matchmaking, match runtime, server runtime hooks/RPC, operations, SDK/developer experience, or distributed runtime. The near-term priority is now `M-148/W-0220 Define agent-native feature request and test workflow`. This next slice must define how user requirements become AI-written specs, acceptance criteria, test plans, tests, implementation, verification, and durable project memory; do not jump directly to startup wiring, broad chat/social modules, matchmaking, match runtime, SDKs, distributed runtime, public announcements, hosted deployments, release artifacts, broad new route families, persistence, delivery guarantees, stream subscriptions, chat rooms, groups, broadcast fanout, or direct Nakama/Pitaya API compatibility before a later bounded work item authorizes it.
