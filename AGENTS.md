...
`M-013 Login Method And Token Format Ratification` is completed by `ADR-0031`. The active authentication-adjacent milestone is now `M-014 Credential And Token Verifier Schema Ratification`, with `W-0075 Define token verifier record schema boundary` as the next ready work item. This next gate may ratify token verifier schema boundaries only; it does not authorize migrations, repositories, PostgreSQL adapters, runtime token validation, handlers, routes, generated authentication shapes, Protobuf messages, WebSocket proof carriers, WebSocket handshake authentication, authentication dependencies, or production authentication behavior.
