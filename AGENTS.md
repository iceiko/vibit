...
`M-129 Storage Objects Behavior Gate` is active. `W-0201 Define storage objects behavior gate` is next-ready. It should define the first general storage-object behavior beyond the inventory proof slice before implementation. Do not implement storage objects runtime behavior, add protocol routes, Protobuf sources or generated output, migrations, dependencies, repository/storage adapters, authentication/session changes, hosted deployments, release artifacts, public announcements, paid promotion, broad product modules, or direct Nakama/Pitaya compatibility without a later bounded work item.
