...
This standard does not add runtime authentication, login handlers, token generation, token parsing, token validation, logout behavior, refresh behavior, credential lookup, credential storage, token storage, session persistence, external identity linking, Protobuf messages, generated Go contract shapes, WebSocket route behavior, WebSocket handshake authentication, migrations, repositories, PostgreSQL adapters, audit persistence, provider dependencies, signing dependencies, password-hashing dependencies, key-management dependencies, or Redis-like dependencies.
...
- `AuthService`, `Authenticator`, `TokenValidator`, `TokenIssuer`, `TokenVerifier`, `TokenRepository`, or `CredentialRepository` runtime implementation.
...
- Credential, token, refresh-token, runtime-session, external-identity, provider-subject, or authentication-audit migrations.
...
Generated Go contract shapes remain deferred. Protobuf wire messages remain deferred. Runtime handlers remain deferred.
