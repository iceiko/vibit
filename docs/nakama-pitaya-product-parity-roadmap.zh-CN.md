...
Authenticated failure-path proof、next capability selection、example client path gate、example client path implementation、follow-up scaffolding selection、feature request scaffolding gate、feature request scaffolding implementation、scaffolded Nakama intake pilot、friends relationship lifecycle gate、friends relationship persistence schema gate、friends relationship migration source、friends relationship repository boundary、storage-neutral friends relationship repository interface implementation、friends relationship PostgreSQL adapter gate、friends relationship PostgreSQL adapter implementation、friends relationship runtime behavior gate、friends relationship runtime behavior implementation、friends relationship protocol route gate、friends relationship protocol route implementation、friends relationship protocol route local proof、post-proof next capability selection、minimum operations inspection surface gate、source-first operations inspection implementation、Pitaya distributed runtime vocabulary map、frontend/backend role map、server-to-server RPC boundary gate、server-to-server RPC source-first map、service discovery boundary gate、service discovery source-first map、distributed group and broadcast boundary/map、cluster-safe session routing boundary/map、next Pitaya-aligned direction selection、route handler pipeline boundary gate、route handler pipeline source-first map、route handler pipeline map direction selection、serializer and message forwarding boundary gate、serializer and message forwarding source-first map、serializer/message-forwarding follow-up direction selection、acceptor and connection lifecycle boundary gate，以及 acceptor and connection lifecycle source-first map 之后，下一个具体工作不应直接跳到 chat、groups、matchmaking、match runtime、SDK publication、hosted demos、distributed runtime implementation、broad social modules、event/audit tables、generated client libraries、新的 protocol shape、admin endpoints、metrics endpoints、observability pipelines、dashboards、frontend/backend server role implementation、server-to-server RPC implementation、remote calls、service discovery implementation、service registries、service selectors、node identity、distributed groups、room broadcast fanout、cluster-safe session routing behavior、route handler implementation、handler routing behavior、handler pipeline behavior、pipeline middleware behavior、serializer behavior、message forwarding behavior、backend route targeting、acceptor behavior、TCP acceptors、WebSocket behavior changes、connection lifecycle behavior changes、session binding behavior 或 kick/disconnect behavior。下一个具体工作只应选择下一项 Pitaya-aligned direction：

```text
next_work_item: W-0268 Define Pitaya-aligned session binding, kick/disconnect, and session data boundary gate
recommended_next_direction: define_pitaya_aligned_session_binding_kick_disconnect_session_data_boundary_gate
primary_product_reference: Nakama
...
- `ADR-0154` 已定义 Pitaya-aligned distributed runtime vocabulary reactivation gate，注册 `runtime.pitaya_aligned_distributed_runtime_vocabulary_reactivation_gate`，并打开 `W-0247 Implement Pitaya-aligned distributed runtime vocabulary source-first map`。
- `ADR-0155` 已实现 `node tools/vibit inspect pitaya-vocabulary --json`，注册 `runtime.pitaya_aligned_distributed_runtime_vocabulary_source_first_map`，并打开 `W-0248 Define Pitaya-aligned frontend/backend role boundary gate`。
- `ADR-0156` 已定义 Pitaya-aligned frontend/backend role boundary gate，注册 `runtime.pitaya_aligned_frontend_backend_role_boundary_gate`，并打开 `W-0249 Implement Pitaya-aligned frontend/backend role source-first map`。
- `ADR-0157` 已实现 `node tools/vibit inspect pitaya-roles --json`，注册 `runtime.pitaya_aligned_frontend_backend_role_source_first_map`，并打开 `W-0250 Define Pitaya-aligned server-to-server RPC boundary gate`。
- `ADR-0158` 已定义 Pitaya-aligned server-to-server RPC boundary gate，注册 `runtime.pitaya_aligned_server_to_server_rpc_boundary_gate`，并打开 `W-0251 Implement Pitaya-aligned server-to-server RPC source-first map`。
- `ADR-0159` 已实现 `node tools/vibit inspect pitaya-rpc --json`，注册 `runtime.pitaya_aligned_server_to_server_rpc_source_first_map`，并打开 `W-0252 Define Pitaya-aligned service discovery boundary gate`。
- `ADR-0160` 已定义 Pitaya-aligned service discovery boundary gate，注册 `runtime.pitaya_aligned_service_discovery_boundary_gate`，并打开 `W-0253 Implement Pitaya-aligned service discovery source-first map`。
- `ADR-0161` 已实现 `node tools/vibit inspect pitaya-discovery --json`，注册 `runtime.pitaya_aligned_service_discovery_source_first_map`，并打开 `W-0254 Define Pitaya-aligned distributed group and broadcast boundary gate`。
- `ADR-0162` 已定义 Pitaya-aligned distributed group and broadcast boundary gate，注册 `runtime.pitaya_aligned_distributed_group_broadcast_boundary_gate`，并打开 `W-0255 Implement Pitaya-aligned distributed group and broadcast source-first map`。
- `ADR-0163` 已实现 `node tools/vibit inspect pitaya-groups --json`，注册 `runtime.pitaya_aligned_distributed_group_broadcast_source_first_map`，并打开 `W-0256 Define Pitaya-aligned cluster-safe session routing boundary gate`。
- `ADR-0164` 已定义 Pitaya-aligned cluster-safe session routing boundary gate，注册 `runtime.pitaya_aligned_cluster_safe_session_routing_boundary_gate`，并打开 `W-0257 Implement Pitaya-aligned cluster-safe session routing source-first map`。
- `ADR-0165` 已实现 `node tools/vibit inspect pitaya-sessions --json`，注册 `runtime.pitaya_aligned_cluster_safe_session_routing_source_first_map`，并打开 `W-0258 Select next Pitaya-aligned direction after cluster-safe session routing map`。
- `ADR-0166` 已选择 `define_pitaya_aligned_route_handler_pipeline_boundary_gate`，注册 `runtime.next_pitaya_aligned_direction_after_cluster_safe_session_routing_map`，并打开 `W-0259 Define Pitaya-aligned route handler pipeline boundary gate`。
- `ADR-0167` 已定义 Pitaya-aligned route handler pipeline boundary gate，注册 `runtime.pitaya_aligned_route_handler_pipeline_boundary_gate`，并打开 `W-0260 Implement Pitaya-aligned route handler pipeline source-first map`。
- `ADR-0168` 已实现 `node tools/vibit inspect pitaya-routes --json` 并注册 `runtime.pitaya_aligned_route_handler_pipeline_source_first_map`；`ADR-0169` 已选择 `define_pitaya_aligned_serializer_message_forwarding_boundary_gate`，注册 `runtime.next_pitaya_aligned_direction_after_route_handler_pipeline_map`，并打开 `W-0262 Define Pitaya-aligned serializer and message forwarding boundary gate`；`ADR-0170` 已定义该 gate，并注册 `runtime.pitaya_aligned_serializer_message_forwarding_boundary_gate`；`ADR-0171` 已实现 `node tools/vibit inspect pitaya-serializer-forwarding --json`，注册 `runtime.pitaya_aligned_serializer_message_forwarding_source_first_map`，并打开 `W-0264 Select next Pitaya-aligned direction after serializer and message forwarding map`；`ADR-0172` 已注册 `runtime.next_pitaya_aligned_direction_after_serializer_message_forwarding_map`，选择 `define_pitaya_aligned_acceptor_connection_lifecycle_boundary_gate`，并打开 `W-0265 Define Pitaya-aligned acceptor and connection lifecycle boundary gate`；`ADR-0173` 已注册 `runtime.pitaya_aligned_acceptor_connection_lifecycle_boundary_gate`，定义该 gate，并打开 `W-0266 Implement Pitaya-aligned acceptor and connection lifecycle source-first map`；`ADR-0174` 已实现 `node tools/vibit inspect pitaya-acceptor-connection --json`，注册 `runtime.pitaya_aligned_acceptor_connection_lifecycle_source_first_map`，并打开 `W-0267 Select next Pitaya-aligned direction after acceptor and connection lifecycle map`；`ADR-0175` 已注册 `runtime.next_pitaya_aligned_direction_after_acceptor_connection_lifecycle_map`，选择 `define_pitaya_aligned_session_binding_kick_disconnect_session_data_boundary_gate`，并打开 `W-0268 Define Pitaya-aligned session binding, kick/disconnect, and session data boundary gate`。
...
- Service discovery source-first map 现在已让未来 Pitaya-aligned service discovery、registry、instance 和 selector vocabulary 可以被检查，再让 groups、parties、chat targeting、invites、matchmaking filters 或 match runtime social context 依赖更大范围。
- 下一项 source-first map 应检查 acceptor 和 connection lifecycle vocabulary，但不添加 acceptor behavior、TCP acceptors、WebSocket behavior changes、connection lifecycle behavior changes、session binding behavior、kick/disconnect behavior、route handler implementation、handler routing behavior、pipeline middleware behavior、serializer behavior、message forwarding behavior、distributed runtime behavior、service discovery behavior、RPC behavior、protocol shape、persistence、dependencies 或 direct compatibility。
- Nakama-first product planning 仍可避免近期 scope 被产品广度和 Pitaya-style distributed implementation 同时拉扯。
- Pitaya-style cluster/RPC/frontend-backend concerns 应继续限制在 vocabulary records 和 source-first maps 内，直到后续明确 implementation work item 授权。
...
`ADR-0192` 已注册 `runtime.pitaya_aligned_handler_module_registration_source_first_map`，实现 `node tools/vibit inspect pitaya-handler-modules --json`，完成 `W-0284 Implement Pitaya-aligned handler module registration source-first map`，打开 `W-0285 Select next Pitaya-aligned direction after handler module registration map`，并继续延后 handler module registration behavior、handler registration behavior、dynamic handler registration、component module loading、runtime endpoint behavior、protocol、persistence、dependencies、distributed runtime、SDK、hosted 和 direct compatibility work。
