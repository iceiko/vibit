...
next_work_item: W-0261 Select next Pitaya-aligned direction after route handler pipeline map
```
...
```text
next_work_item: W-0261 Select next Pitaya-aligned direction after route handler pipeline map
recommended_next_direction: select_next_pitaya_aligned_direction_after_route_handler_pipeline_map
primary_product_reference: Nakama
pitaya_reference_status: route_handler_pipeline_source_first_map_implemented
...
- `ADR-0168` 已实现 `node tools/vibit inspect pitaya-routes --json`，注册 `runtime.pitaya_aligned_route_handler_pipeline_source_first_map`，并打开 `W-0261 Select next Pitaya-aligned direction after route handler pipeline map`。
