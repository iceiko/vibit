...
next_work_item: W-0263 Implement Pitaya-aligned serializer and message forwarding source-first map
```
...
```text
next_work_item: W-0263 Implement Pitaya-aligned serializer and message forwarding source-first map
recommended_next_direction: implement_pitaya_aligned_serializer_message_forwarding_source_first_map
primary_product_reference: Nakama
pitaya_reference_status: serializer_message_forwarding_vocabulary_boundary_defined_for_future_architecture_planning
...
- `ADR-0168` 已实现 `node tools/vibit inspect pitaya-routes --json` 并注册 `runtime.pitaya_aligned_route_handler_pipeline_source_first_map`；`ADR-0169` 已选择 `define_pitaya_aligned_serializer_message_forwarding_boundary_gate`，注册 `runtime.next_pitaya_aligned_direction_after_route_handler_pipeline_map`，并打开 `W-0262 Define Pitaya-aligned serializer and message forwarding boundary gate`；`ADR-0170` 已定义该 gate，注册 `runtime.pitaya_aligned_serializer_message_forwarding_boundary_gate`，并打开 `W-0263 Implement Pitaya-aligned serializer and message forwarding source-first map`。
