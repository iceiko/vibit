...
next_work_item: W-0258 Select next Pitaya-aligned direction after cluster-safe session routing map
```
...
- 下一项 source-first selection 应在 cluster-safe session routing 之后选择下一个 bounded Pitaya-aligned direction，但不添加 group implementation、group membership registries、room broadcast fanout、delivery guarantees、distributed runtime behavior、service discovery behavior、RPC behavior、protocol shape、persistence、dependencies 或 direct compatibility。
