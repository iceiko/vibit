...
Release execution、first alpha discovery/feedback、prototype-ready execution plan、storage objects path、realtime outbound delivery path、Nakama-first direction、agent-native feature request/test workflow、Nakama-aligned presence/status workflow pilot、presence/status local proof hardening、authenticated gameplay failure-path verification、next Nakama prototype-ready capability selection、local alpha example client path gate、local alpha example client path implementation、feature request scaffolding selection、feature request scaffolding gate、feature request scaffolding implementation、scaffolded Nakama feature request intake pilot、friends relationship lifecycle gate、friends relationship persistence schema gate、friends relationship migration source、friends relationship repository boundary、friends relationship repository interface implementation、friends relationship PostgreSQL adapter gate、friends relationship PostgreSQL adapter implementation、friends relationship runtime behavior gate、friends relationship runtime behavior implementation、friends relationship protocol route gate、friends relationship protocol route implementation、friends relationship protocol route local proof、minimum operations inspection surface gate 和 minimum operations inspection source-first surface implementation 继续记录在既有 artifacts 中。Agent-native feature request and test workflow 已记录在 `docs/agent-native-feature-request-test-workflow.md` 和 `ADR-0128`，scaffolded intake pilot decision 已记录在 `ADR-0138`，friends lifecycle gate decision 已记录在 `ADR-0139`，friends persistence schema gate decision 已记录在 `ADR-0140`，friends migration source decision 已记录在 `ADR-0141`，friends repository boundary decision 已记录在 `ADR-0142`，friends repository interface implementation decision 已记录在 `ADR-0143`，检查规则是 `runtime.friends_relationship_repository_interface_implementation`；friends PostgreSQL adapter gate decision 已记录在 `ADR-0144`，检查规则是 `runtime.friends_relationship_postgresql_adapter_gate`；friends PostgreSQL adapter implementation decision 已记录在 `ADR-0145`，检查规则是 `runtime.friends_relationship_postgresql_adapter_implementation`；friends runtime behavior gate decision 已记录在 `ADR-0146`，检查规则是 `runtime.friends_relationship_runtime_behavior_gate`；friends runtime behavior implementation decision 已记录在 `ADR-0147`，检查规则是 `runtime.friends_relationship_runtime_behavior_implementation`；friends protocol route gate decision 已记录在 `ADR-0148`，检查规则是 `runtime.friends_relationship_protocol_route_gate`；friends protocol route implementation decision 已记录在 `ADR-0149`，检查规则是 `runtime.friends_relationship_protocol_route_implementation`；friends protocol route local proof decision 已记录在 `ADR-0150`，检查规则是 `runtime.friends_relationship_protocol_route_local_proof`；minimum operations inspection surface gate 已记录在 `ADR-0152`，检查规则是 `runtime.minimum_operations_inspection_surface_gate`；source-first operations inspection implementation 已记录在 `ADR-0153`，检查规则是 `runtime.minimum_operations_inspection_source_first_surface_implementation`。下一步 work 是 `W-0246 Define Pitaya-aligned distributed runtime vocabulary reactivation gate`；next_direction: implement_pitaya_aligned_distributed_runtime_vocabulary_source_first_map。
