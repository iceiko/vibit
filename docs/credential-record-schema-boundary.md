...
The logical table name is ratified and the SQL source now exists under `runtime/migrations/postgres/`. The migration source does not authorize repositories, adapters, runtime lookup, handlers, routes, generated authentication output, Protobuf messages, WebSocket behavior, authentication dependencies, or production authentication behavior.
