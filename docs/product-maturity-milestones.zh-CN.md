...
next_direction: implement_pitaya_aligned_distributed_runtime_vocabulary_source_first_map
...
```text
W-0247 Implement Pitaya-aligned distributed runtime vocabulary source-first map
```

Agent-native feature request and test workflow 已记录在 `docs/agent-native-feature-request-test-workflow.md` 和 `ADR-0128`。`ADR-0138` 已完成 scaffolded Nakama intake 并选择 `friends_groups_and_parties`，`ADR-0139` 已定义 friends relationship lifecycle gate，`ADR-0140` 已定义 friends relationship persistence schema gate，`ADR-0141` 已添加 `runtime/migrations/postgres/000007_create_friend_relationships.sql`，`decisions/ADR-0142-friends-relationship-repository-boundary.md` 已定义 friends relationship repository boundary，`ADR-0143` 已实现 storage-neutral friends relationship repository interface，检查规则是 `runtime.friends_relationship_repository_interface_implementation`，`decisions/ADR-0144-friends-relationship-postgresql-adapter-gate.md` 已定义 friends relationship PostgreSQL adapter gate，检查规则是 `runtime.friends_relationship_postgresql_adapter_gate`，`decisions/ADR-0145-friends-relationship-postgresql-adapter-implementation.md` 已实现 friends relationship PostgreSQL adapter，检查规则是 `runtime.friends_relationship_postgresql_adapter_implementation`，`decisions/ADR-0146-friends-relationship-runtime-behavior-gate.md` 已定义 friends relationship runtime behavior gate，检查规则是 `runtime.friends_relationship_runtime_behavior_gate`，`decisions/ADR-0147-friends-relationship-runtime-behavior-implementation.md` 已实现 friends relationship runtime behavior，检查规则是 `runtime.friends_relationship_runtime_behavior_implementation`，`decisions/ADR-0148-friends-relationship-protocol-route-gate.md` 已定义 friends relationship protocol route gate，检查规则是 `runtime.friends_relationship_protocol_route_gate`，`decisions/ADR-0149-friends-relationship-protocol-route-implementation.md` 已实现 friends relationship protocol route，检查规则是 `runtime.friends_relationship_protocol_route_implementation`，`decisions/ADR-0150-friends-relationship-protocol-route-local-proof.md` 已证明 friends relationship protocol route local flow，检查规则是 `runtime.friends_relationship_protocol_route_local_proof`，`decisions/ADR-0152-minimum-operations-inspection-surface-gate.md` 已定义 minimum operations inspection gate，检查规则是 `runtime.minimum_operations_inspection_surface_gate`，`decisions/ADR-0153-minimum-operations-inspection-source-first-surface-implementation.md` 已实现 source-first operations inspection，检查规则是 `runtime.minimum_operations_inspection_source_first_surface_implementation`，`decisions/ADR-0154-pitaya-aligned-distributed-runtime-vocabulary-reactivation-gate.md` 已定义 Pitaya-aligned distributed runtime vocabulary reactivation gate，检查规则是 `runtime.pitaya_aligned_distributed_runtime_vocabulary_reactivation_gate`。下一项 work 是 `W-0247 Implement Pitaya-aligned distributed runtime vocabulary source-first map`；它只应实现 source-first vocabulary map，同时继续延后 protocol shape changes、service behavior changes、repository interface changes、PostgreSQL adapter changes、migrations、dependencies、stream subscriptions、chat rooms、groups、parties、broadcast fanout、delivery guarantees、distributed runtime implementation、frontend/backend server roles、server-to-server RPC、remote calls、service discovery、cluster-safe session routing、matchmaking、match runtime、SDK publication、hosted deployments、release artifacts、public announcements、event/audit tables、admin endpoints、metrics endpoints、observability pipelines、dashboards 和 direct compatibility，除非后续明确 work item 授权。
