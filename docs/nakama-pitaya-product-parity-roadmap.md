...
- `ADR-0162` defined the Pitaya-aligned distributed group and broadcast boundary gate, registered `runtime.pitaya_aligned_distributed_group_broadcast_boundary_gate`, and opened `W-0255 Implement Pitaya-aligned distributed group and broadcast source-first map`.
- `ADR-0163` implemented `node tools/vibit inspect pitaya-groups --json`, registered `runtime.pitaya_aligned_distributed_group_broadcast_source_first_map`, and opened `W-0256 Define Pitaya-aligned cluster-safe session routing boundary gate`.
...
- `ADR-0168` implemented `node tools/vibit inspect pitaya-routes --json` and registered `runtime.pitaya_aligned_route_handler_pipeline_source_first_map`; `ADR-0169` selected `define_pitaya_aligned_serializer_message_forwarding_boundary_gate`, registered `runtime.next_pitaya_aligned_direction_after_route_handler_pipeline_map`, and opened `W-0262 Define Pitaya-aligned serializer and message forwarding boundary gate`; `ADR-0170` defined that gate and registered `runtime.pitaya_aligned_serializer_message_forwarding_boundary_gate`; `ADR-0171` implemented `node tools/vibit inspect pitaya-serializer-forwarding --json`, registered `runtime.pitaya_aligned_serializer_message_forwarding_source_first_map`, and opened `W-0264 Select next Pitaya-aligned direction after serializer and message forwarding map`; `ADR-0172` registered `runtime.next_pitaya_aligned_direction_after_serializer_message_forwarding_map`, selected `define_pitaya_aligned_acceptor_connection_lifecycle_boundary_gate`, and opened `W-0265 Define Pitaya-aligned acceptor and connection lifecycle boundary gate`; `ADR-0173` registered `runtime.pitaya_aligned_acceptor_connection_lifecycle_boundary_gate`, defined that gate, and opened `W-0266 Implement Pitaya-aligned acceptor and connection lifecycle source-first map`; `ADR-0174` implemented `node tools/vibit inspect pitaya-acceptor-connection --json`, registered `runtime.pitaya_aligned_acceptor_connection_lifecycle_source_first_map`, and opened `W-0267 Select next Pitaya-aligned direction after acceptor and connection lifecycle map`; `ADR-0175` registered `runtime.next_pitaya_aligned_direction_after_acceptor_connection_lifecycle_map`, selected `define_pitaya_aligned_session_binding_kick_disconnect_session_data_boundary_gate`, and opened `W-0268 Define Pitaya-aligned session binding, kick/disconnect, and session data boundary gate`.
...
- The next source-first map should inspect acceptor and connection lifecycle vocabulary without adding acceptor behavior, TCP acceptors, WebSocket behavior changes, connection lifecycle behavior changes, session binding behavior, kick/disconnect behavior, route handler implementation, handler routing behavior, pipeline middleware behavior, serializer behavior, message forwarding behavior, distributed runtime behavior, service discovery behavior, RPC behavior, protocol shape, persistence, dependencies, or direct compatibility.
...
`ADR-0185` registered `runtime.pitaya_aligned_dashboard_admin_operations_boundary_gate`, completed `W-0277 Define Pitaya-aligned dashboard and admin operations boundary gate`, defined gate-only dashboard/admin operations vocabulary, and keeps runtime endpoint behavior, metrics endpoints, tracing pipelines, observability pipelines, dashboards, admin console behavior, protocol, persistence, dependencies, distributed runtime, SDK, hosted, and direct compatibility work deferred.
