...
- `ADR-0168` implemented `node tools/vibit inspect pitaya-routes --json` and registered `runtime.pitaya_aligned_route_handler_pipeline_source_first_map`; `ADR-0169` selected `define_pitaya_aligned_serializer_message_forwarding_boundary_gate`, registered `runtime.next_pitaya_aligned_direction_after_route_handler_pipeline_map`, and opened `W-0262 Define Pitaya-aligned serializer and message forwarding boundary gate`.
